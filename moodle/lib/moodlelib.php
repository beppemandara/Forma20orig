 ...
 *
 * AKTIVE: Modified to authenticates a user against all the enabled authentication plugins.
 *  TODO:   use field mdl_user.auth as an ordered list of comma separated auth plugins.
 *                Modify user form in order to manage multi plugin selection. 
 *                (Pay attention to nologin auth plugin and disable user autocreation from auth plugin).
 *
...
function authenticate_user_login($username, $password, $ignorelockout=false, &$failurereason=null) {
    ...
    if ($user) {
        ...
        //AK: hack to enable multi authentication in moodle.
        //$auths = array($auth);
	$auths = $authsenabled;
...
function get_course_display_name_for_list($course) {
    ...
    if (!empty($CFG->courselistshortnames)) {
 	...
 	if ($course->idnumber)
            return $course->idnumber." - ".$course->fullname;
 	else
