...
class course_enrolment_manager {
    ...
    public function get_total_other_users($extrafilter=null) {
        global $DB;
        if ($this->totalotherusers === null) {
            ...
            if(!is_null($extrafilter) && isset($extrafilter['extrasql']) && strlen($extrafilter['extrasql'])>1){
            	$params = $params + $extrafilter['params'];
            	if(strrpos($extrafilter['extrasql'], "viewableorganisationid")===false)
            	    $extrasql = " JOIN (select uu.* FROM {user} uu WHERE ".$extrafilter['extrasql'].") u ON u.id = ra.userid";
            	else
            	    $extrasql = " JOIN (select uu.*,os.viewableorganisationid  FROM {user} uu, {org_assignment} os WHERE os.userid=uu.id AND ".$extrafilter['extrasql'].") u ON u.id = ra.userid";
           }
           else 
           	$extrasql = "JOIN {user} u ON u.id = ra.userid";
    ...
    public function get_other_users($sort, $direction='ASC', $page=0, $perpage=25,$extrafilter=null) {
        global $DB;
        if ($direction !== 'ASC') {
            $direction = 'DESC';
        }
...
// 20171002
            ...
            $sql = "SELECT ra.id as raid, ra.contextid, ra.component, ctx.contextlevel, ra.roleid, u.*, ue.lastseen
                    , org_ass.viewableorganisationid, concat(org.idnumber,' - ',org.fullname) org_name 
                    FROM 
                    {org} org RIGHT JOIN  {org_assignment} org_ass ON org_ass.viewableorganisationid = org.id,
                    {role_assignments} ra
                    $extrasql 
                    JOIN {context} ctx ON ra.contextid = ctx.id 
               LEFT JOIN ( 
    ...
    public function search_other_users($search='', $searchanywhere=false, $page=0, $perpage=25) {
        ...
        // override del metodo in core.php
        return my_search_other_users($search, $searchanywhere, $this->context->id, $page, $perpage);
        // punto MAI raggiungibile
    ...
    public function get_other_users_for_display(core_enrol_renderer $renderer, moodle_url $pageurl, $sort, $direction, $page, $perpage,$extrafilter=null) {

        //$userroles = $this->get_other_users($sort, $direction, $page, $perpage);
        $userroles = $this->get_other_users($sort, $direction, $page, $perpage,$extrafilter);
