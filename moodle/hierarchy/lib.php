<?php // $Id: lib.php 1294 2014-07-04 09:47:00Z l.moretto $
...
class hierarchy {
    ...
    function get_framework($id = 0, $noframeworkok = false) {
        ...
        if ($id == 0) {
                // Get frameworks
                $frameworks = $DB->get_records_select($this->shortprefix.'_framework', '1 = 1 ORDER BY sortorder ASC');
    ...
    function get_depth_by_id($id) {
        global $DB;
        return $DB->get_record($this->shortprefix.'_depth', array('id' => $id));
    }
...
//            return get_records($this->shortprefix.'_framework', '', '', 'sortorder, fullname');
    ...
    function get_depths($extra_data=array()) {
        global $DB;
        if (!count($extra_data)) {
           return $DB->get_records($this->shortprefix.'_depth', array('frameworkid' => $this->frameworkid), 'depthlevel');
        }
    ...
    function get_custom_field_category_by_id($id) {
        global $DB;
        return $DB->get_record($this->shortprefix.'_depth_info_category', array('id' => $id));
    }
    ...
    function get_items() {
        global $DB;
        return $DB->get_records($this->shortprefix, array('frameworkid' => $this->frameworkid), 'path, sortorder, fullname');
    }
    ...
    function get_items_by_parent($parentid=false) {
        ...
        if ($parentid) {
            ...
            return $DB->get_records_select($this->shortprefix, "parentid = {$parentid}", array(), 'frameworkid, sortorder, fullname');
        }
        else {
    ...
    function get_all_root_items($all=false) {
        ...
        if(empty($this->frameworkid) || $all) {
            ...
            return $DB->get_records($this->shortprefix, array('parentid' => 0), 'frameworkid, sortorder, fullname');
        } else {
            // root level items for current framework only
            $fwid = $this->frameworkid;
            return $DB->get_records_select($this->shortprefix, "parentid = 0 AND frameworkid = $fwid", array(), 'sortorder, fullname');
        }
    }
    ...
    function get_item_adjacent_peer($item, $above=true) {
        ...
        $dest = $DB->get_record_sql($sql, array(), IGNORE_MULTIPLE);
        if($dest) {
            return $dest->id;
        } else {
...
//        $myurl = $_SERVER['PHP_SELF'].'?type='.$options['type'].',frameworkid='.$options['frameworkid'].',spage='.$options['spage'].',edit='.$options['edit'];

        return $OUTPUT->single_button(new moodle_url($_SERVER['PHP_SELF'], $options), $label, 'get');
//        return print_single_button($_SERVER['PHP_SELF'], $options, $label, 'get', '', true);
    ...
    function display_framework_selector($page = 'index.php', $simple = false) {
        ...
        if (!$simple) {
            ...
            popup_form($CFG->wwwroot.'/hierarchy/'.$page.'?type='.$this->prefix.'&frameworkid=', $fwoptions, 'switchframework', $this->frameworkid, '', '', '', false, 'self', get_string('switchframework', 'local_f2_domains'));
//            echo $OUTPUT->single_select(new moodle_url($CFG->wwwroot.'/hierarchy/'.$page, ).'?type='.$this->prefix.'&frameworkid=', $fwoptions, 'switchframework', $this->frameworkid, '', '', '', false, 'self', get_string('switchframework', 'local_f2_domains'));
    ...
    function display_add_item_button($spage=0) {
        ...
        echo $OUTPUT->single_button(new moodle_url($CFG->wwwroot.'/hierarchy/item/edit.php', $options), get_string('addnew'.$this->prefix, 'local_f2_domains'), 'get');
    }
    ...
    function move_item($id, $up) {
        ...
        $source = $DB->get_record($this->shortprefix, array('id' => $id));
        ...
        if(!$destid) {
            ...
            notify(get_string('error:couldnotmoveitemnopeer','local_f2_domains',$this->prefix));
            return false;
        }
        ...
        try {
            ...
            foreach($sourcetree as $item) {
                ...
                if($newso < 0) {
                    ...
                    $transaction->rollback(new Exception(get_string('error:badsortorder','local_f2_domains',$this->prefix)));
                    notify(get_string('error:badsortorder','local_f2_domains',$this->prefix));
                    return false;
                } else {
                    $status = $status && $DB->set_field($this->shortprefix, 'sortorder', $newso, array('id' => $id));
                }
            }
            ...
            if($status) {
                ...
                $transaction->rollback(new Exception(get_string('error:couldnotmoveitem','local_f2_domains',$this->prefix)));
                notify(get_string('error:couldnotmoveitem','local_f2_domains',$this->prefix));
                return false;
            }
        } catch (Exception $e) {
    ...
    function hide_item($id) {
        ...
        if ($item) {
            $visible = 0;
            if (!$DB->set_field($this->shortprefix, 'visible', $visible, array('id' => $item->id))) {
    ...
    function hide_framework($id) {
        ...
        if ($framework) {
            $visible = 0;
            if (!$DB->set_field($this->shortprefix.'_framework', 'visible', $visible, array('id' => $framework->id))) {
...
//        print_r('framework: '.$this->shortprefix.'_framework');
//        print_r('   id: '.$id);
        ...
        if ($up) {
            ...
            $swap = $DB->get_record_sql(
                    ...
                    FROM {$CFG->prefix}{$this->shortprefix}_framework
                    WHERE sortorder < {$move->sortorder}
                    ORDER BY sortorder DESC", array(), IGNORE_MULTIPLE
            );
//                    print_r('   swap: ');
//        var_dump($swap);
        } else {
            $swap = $DB->get_record_sql(
                    ...
                    FROM {$CFG->prefix}{$this->shortprefix}_framework
                    WHERE sortorder > {$move->sortorder}
                    ORDER BY sortorder ASC", array(), IGNORE_MULTIPLE
            );
//                     print_r('   swap: ');
//        var_dump($swap);
        }
        ...
        if ($move && $swap) {
            try {
                $transaction = $DB->start_delegated_transaction();
                if (!(    $DB->set_field($this->shortprefix.'_framework', 'sortorder', $sortoffset, array('id' => $swap->id))
                ...
                $transaction->rollback(new Exception('error during move framework operation'));
            }
        }
        return false;
    }
    ...
    function delete_framework_item($id, $usetransaction = true) {
        ...
        try {
            ...
            $children = $DB->get_records($this->shortprefix, array('parentid' => $id));
            ...
            if($DB->delete_records($this->shortprefix.'_depth_info_data', array($this->prefix.'id' => $id))) {
                // Finally delete this item
                if($DB->delete_records($this->shortprefix, array('id' => $id))) {
                    if ( $usetransaction ){
                        $transaction->allow_commit();
                    }
                    return true;
    ...
    function delete_framework() {
        ...
        if ($items) {
            foreach ($items as $item) {
                ...
                $DB->delete_records($this->shortprefix.'_depth_info_data', array($this->prefix.'id' => $item->id));
            }
        }
        ...
        $DB->delete_records($this->shortprefix.'_depth', array('frameworkid' => $this->frameworkid));

        // Delete all items in the framework
        $DB->delete_records($this->shortprefix, array('frameworkid' => $this->frameworkid));
        ...
        $sortorder = 1;
        $records = $DB->get_records_sql("SELECT id FROM {$CFG->prefix}{$this->shortprefix}_framework ORDER BY sortorder ASC");
        if (is_array($records)) {
    ...
    function is_safe_to_delete_depth($id) {
        ...
        if ( !$DB->get_record($this->shortprefix.'_depth', array('id' => $id)) ){
            return 'deletedepthnosuchdepth';
        }

        if ( $DB->count_records($this->shortprefix, array('depthid' => $id)) > 0 ){
    ...
    function delete_depth($id) {
        ...
        if ( $safe_or_not === true ){
            ...
            $DB->delete_records($this->shortprefix.'_depth', array('id' => $id));
        }
        return $safe_or_not;
    }
    ...
    function delete_depth_metadata($id) {
        ...
        $DB->delete_records($this->shortprefix.'_depth_info_field', array('depthid' => $id));

        // Delete all info categories in a depth
        $DB->delete_records($this->shortprefix.'_depth_info_category', array('depthid' => $id));
    ...
    function get_item_data($item, $cols = NULL) {
        ...
        foreach ($cols as $datatype) {
            $data[] = array(
                'title' => get_string($datatype.'view', 'local_f2_domains', $depth->fullname),
                'value' => $item->$datatype
            );
        }
