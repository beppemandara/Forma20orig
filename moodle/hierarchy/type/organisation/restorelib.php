...
function organisation_restore_framework($fwinfo, $options, $backup_unique_code) {
    ...
    $newid = $DB->insert_record('org_framework',$framework);
...
//        }
        ...
        $newid = $DB->insert_record('org_depth',$depth);
...
function organisation_restore_custom_category($olddepthid, $newdepthid, $depthinfo, $options, $backup_unique_code) {
    ...
    for($i=0;$i < sizeof($categories); $i++) {
        ...
        $newid = $DB->insert_record('org_depth_info_category', $category);
        if($newid) {
...
function organisation_restore_custom_field($oldcatid, $newcatid, $catinfo, $options, $backup_unique_code) {
    ...
    for($i=0;$i < sizeof($fields); $i++) {
        ...
        $newid = $DB->insert_record('org_depth_info_field', $field);
        if($newid) {
...
//        $oldid = backup_todb($organisation_info['#']['ID']['0']['#']);
        ...
        $organisation->id = backup_todb($organisation_info['#']['ID']['0']['#']);
...
//        $newid = $DB->insert_record('org',$organisation);
        $params = (array)$organisation;
        $fields = implode(',', array_keys($params));
        $qms    = array_fill(0, count($params), '?');
        $qms    = implode(',', $qms);

        $DB->execute("INSERT INTO {$CFG->prefix}org ($fields) VALUES($qms)", $params);
...
function organisation_restore_custom_data($oldorgid, $neworgid, $orginfo, $options, $backup_unique_code) {
    ...
    for($i=0; $i <sizeof($values); $i++) {
        ...
        $newid = $DB->insert_record('org_depth_info_data',$value);
        if($newid) {
