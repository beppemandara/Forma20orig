...
//        if (! $status = restore_unzip ($CFG->dataroot."/temp/backup/".$backup_unique_code."/".basename($file))) {
        if (! $status = unzip_file($CFG->dataroot."/temp/backup/".$backup_unique_code."/".basename($file))) {
...
function match_users($info, $backup_unique_code) {
    ...
    $DB->delete_records($tempusers, array('backup_unique_code' => $backup_unique_code));
    ...
    if(isset($info['MOODLE_BACKUP']['#']['COURSE']['0']['#']['USERS']['0']['#']['USER'])) {
        ...
        foreach($users as $user) {
            ...
            $DB->insert_record($tempusers,$todb); 
        }
    } else {
        return false;
    }
...
function match_items($frameworkinfo, $hname, $fwid, $tempitems, $backup_unique_code) {
    ...
    if (isset($frameworkinfo['#'][$itemplural]['0']['#'][$itemsingle])) {
        ...
        foreach ($items AS $item) {
            ...
            $todb->backup_unique_code = $backup_unique_code;
            $res = $DB->insert_record($tempitems, $todb);
        }
    } else {
        return false;
    }
...
//    require_once("$CFG->libdir/xmldb/classes/XMLDBTable.class.php");
    ...
    $table->add_field('oid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED,
                          XMLDB_NOTNULL, null, null, null);
    $table->add_field('frameworkid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED,
                          XMLDB_NOTNULL, null, null, null);
    $table->add_field('idnumber', XMLDB_TYPE_CHAR, '100', null,
                         null, null, null, null);
    $table->add_field('shortname', XMLDB_TYPE_CHAR, '100', null,
                         XMLDB_NOTNULL, null, null, null);
    $table->add_field('fullname', XMLDB_TYPE_TEXT, 'medium', null,
                         XMLDB_NOTNULL, null, null, null);
    $table->add_field('backup_unique_code', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED,
                         XMLDB_NOTNULL, null, null, null);
    /// Adding keys
    ...
    return $DB->get_manager()->create_table($table, true, false);
...
//        }
    ...
    if($DB->update_record($desttable, $pathupdate)) {
        return $newpath;
    } else {
        return false;
    }
...
function get_matches($xmlinfo, $matchfields, $tablename, $where=null) {
    ...
    foreach ($matchfields AS $matchfield) {
        ...
        $queries[$matchfield] = $DB->get_records_sql($sql);
    }
...
function get_sortorder($table, $sortorder) {
    global $CFG, $DB;
    $matches = $DB->count_records($table, array('sortorder' => $sortorder));
    if($matches) {
...
// QUESTE FUNZIONI SONO STATE AGGIUNTE
