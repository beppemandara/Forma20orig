...
if ($id == 0) {
    ...
    require_capability('local/f2_domains:create'.$type.'depth', $context);
    ...
    $depth->depthlevel = $DB->get_field($shortprefix.'_depth', 'MAX(depthlevel) + 1', array('frameworkid' => $frameworkid));
    if (!$depth->depthlevel) {
        $depth->depthlevel = 1;
    }
    ...
    require_capability('local/f2_domains:update'.$type.'depth', $context);
...
// Load framework
if (!$framework = $DB->get_record($shortprefix.'_framework', array('id' => $depth->frameworkid))) {
...
// update data
    ...
    if ($depthnew->id == 0) {
        ...
        if (!$depthnew->id = $DB->insert_record($shortprefix.'_depth', $depthnew)) {
        ...
        if (!$DB->insert_record($shortprefix.'_depth_info_category', $depthinfocategorynew)) {
            print_error('Error creating '.$type.' depth info category record');
        }

    // Existing depth level
    } else {
        if (!$DB->update_record($shortprefix.'_depth', $depthnew)) {
            print_error('Error updating '.$type.' depth level record');
        }
    }

    // Reload from db
    $depthnew = $DB->get_record($shortprefix.'_depth', array('id' => $depthnew->id));
...
$navlinks[] = array('name'=>format_string($framework->fullname),
                    ...
                    'type'=>'misc');
if ($id == 0) {
    $navlinks[] = array('name'=>get_string('adddepthlevel', 'local_f2_domains'), 'link'=>'', 'type'=>'misc');
} else {
    $navlinks[] = array('name'=>get_string('editgeneric', 'local_f2_domains', format_string($depth->fullname)), 'link'=>'', 'type'=>'misc');
}

echo $OUTPUT->header('', $navlinks);

if ($depth->id == 0) {
    echo $OUTPUT->heading(get_string('adddepthlevel', 'local_f2_domains'));
} else {
    echo $OUTPUT->heading(get_string('editgeneric', 'local_f2_domains', format_string($depth->fullname)));
}
