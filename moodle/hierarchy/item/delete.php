...
require_capability('local/f2_domains:delete'.$type, $sitecontext);
...
if (!$delete) {
    $strdelete = get_string('deletecheck', 'local_f2_domains');

    echo $OUTPUT->confirm("$strdelete<br /><br />" . format_string($item->fullname),
...
echo $OUTPUT->heading(get_string('deleted'.$type, 'local_f2_domains', format_string($item->fullname)));
