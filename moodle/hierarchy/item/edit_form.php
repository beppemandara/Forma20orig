    ...
    function definition() {
        ...
        if ($items) {
            ...
            foreach ($items as $parent) {
                ...
                $breadcrumbs = array_slice($breadcrumbs, 0, ($depths[$parent->depthid]->depthlevel - 1), true);
                $breadcrumbs[$depths[$parent->depthid]->depthlevel - 1] = $parent->fullname;
        ...
        $mform->addElement('text', 'framework', get_string('framework', 'local_f2_domains'));
        $mform->setType('framework', PARAM_TEXT);
//        $mform->setHelpButton('framework', array($type.'framework', get_string('framework', 'local_f2_domains')), true);
        $mform->addHelpButton('framework', 'framework', 'local_f2_domains');
        $mform->hardFreeze('framework');

        $mform->addElement('select', 'parentid', get_string('parent', 'local_f2_domains'), $parents);
//        $mform->setHelpButton('parentid', array($type.'parent', get_string('parent', 'local_f2_domains')), true);
        $mform->addHelpButton('parentid', 'parent', 'local_f2_domains');
        ...
        $mform->addElement('text', 'fullname', get_string('fullname', 'local_f2_domains'), 'maxlength="254" size="50"');
//        $mform->setHelpButton('fullname', array($type.'fullname', get_string('fullname', 'local_f2_domains')), true);
        $mform->addHelpButton('fullname', 'fullname_title', 'local_f2_domains');
        $mform->addRule('fullname', get_string('missingfullname', 'local_f2_domains'), 'required', null);
        $mform->setType('fullname', PARAM_MULTILANG);

        $mform->addElement('text', 'shortname', get_string('shortname', 'local_f2_domains'), 'maxlength="100" size="20"');
//        $mform->setHelpButton('shortname', array($type.'shortname', get_string('shortname', 'local_f2_domains')), true);
        $mform->addHelpButton('shortname', 'shortname_title', 'local_f2_domains');
        $mform->addRule('shortname', get_string('missingshortname', 'local_f2_domains'), 'required', null);
        $mform->setType('shortname', PARAM_MULTILANG);

        $mform->addElement('text', 'idnumber', get_string('idnumber', 'local_f2_domains'), 'maxlength="100"  size="10"');
//        $mform->setHelpButton('idnumber', array($type.'idnumber', get_string('idnumber', 'local_f2_domains')), true);
        $mform->addHelpButton('idnumber', 'idnumber_title', 'local_f2_domains');
...
//            $mform->setHelpButton('description', array('text', get_string('helptext')), true);
            $mform->addHelpButton('description', 'description_title', 'local_f2_domains');
    ...
    function validation($itemnew, $files) {
        ...
        $item    = $DB->get_record(hierarchy::get_short_prefix($itemnew->type), array('id' => $itemnew->id));
