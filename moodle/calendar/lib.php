    ...
    for($day = 1; $day <= $display->maxdays; ++$day, ++$dayweek) {
        ...
        if(isset($eventsbyday[$day])) {
            ...
            foreach($eventsbyday[$day] as $eventid) {
                ...
                $get_course_event = $DB->get_record_sql("SELECT c.idnumber,c.fullname FROM {course} c, {facetoface} f WHERE f.course = c.id AND f.id = ".$event->instance);
                ...
                $popupcontent .= $get_course_event->idnumber.' - '.$get_course_event->fullname; // 2017 10 02
            ...
            /* Inizio AK
            Se la sessione e' nascosta, dal calendario non deve essere possibile andare alla pagina della sessione
            */
            //$cell = html_writer::link($dayhref, $day, array('id' => $popupid));
            if($event->eventtype == 'facetofacebooking' && $event->visible == 0)
...
function calendar_get_events($tstart, $tend, $users, $groups, $courses, $withduration=true, $ignorehidden=true) {
    ...
    if ($ignorehidden) {
        ...
        /* Inizio AK
        E' stata sostituita la clausula commentata in alto
        perche' nonostante la sessione sia nascosta, nel calendario deve essere visualizzato l'evento
    	*/
        $whereclause .= ' AND (visible = 1 OR eventtype = \'facetofacebooking\')'; // $whereclause MODIFICATO
        // Fine AK
    }
