...
// AKTIVE: verifico che la risorsa che si sta andando a scaricare sia di tipo "Scheda Progetto", 
// in questo caso l'utente non deve necessariamente essere iscritto al corso
if ($DB->record_exists_sql("SELECT u.name FROM {$CFG->prefix}course_modules cm
                            JOIN {$CFG->prefix}modules m ON m.id = cm.module
                            JOIN {$CFG->prefix}url u ON u.id = cm.instance
                            WHERE m.name = 'url' AND u.name = 'Scheda Progetto'")) {
    require_login();
} else {
