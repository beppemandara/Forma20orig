...
class mod_facetoface_renderer extends plugin_renderer_base {
    /**
     * AK-LS: aggiunta di discriminazione sul sito di visualizzazione/operazioni
     * Builds session list table given an array of sessions
     */
    public function print_session_list_table($customfields, $sessions, $viewattendees, $editsessions, $editsessionsdates) {
        $output = '';
        ...
        foreach ($sessions as $session) {
            ...
            } else { // AK-LS: privilegi di modifica ridotti : impostazione parametro m(minus)
            	if ($editsessionsdates) {
            		$options .= $this->output->action_icon(new moodle_url('sessions.php', array('s' => $session->id, 'm' => 1)), new pix_icon('t/edit', get_string('edit', 'facetoface')), null, array('title' => get_string('editsession', 'facetoface'))) . ' ';
            		$options .= $this->output->action_icon(new moodle_url('sessions.php', array('s' => $session->id, 'c' => 1)), new pix_icon('t/copy', get_string('copy', 'facetoface')), null, array('title' => get_string('copysession', 'facetoface'))) . ' ';
            		$options .= $this->output->action_icon(new moodle_url('sessions.php', array('s' => $session->id, 'd' => 1)), new pix_icon('t/delete', get_string('delete', 'facetoface')), null, array('title' => get_string('deletesession', 'facetoface'))) . ' ';
            		$options .= html_writer::empty_tag('br');
            	}
            ...
            // AK-LS: bloccata l'autoiscrizione
            // GM ripristinata l'autoiscrizione
            /**/if ($isbookedsession) {
