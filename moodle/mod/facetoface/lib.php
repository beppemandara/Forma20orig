<?php
// $Id: lib.php 1327 2014-12-01 09:43:53Z l.moretto $
/*
...
/*
 * AK-LS: upgrade face-to-face con nuovi customfields
*/
define('CUSTOMFIELD_TYPE_DATETIMESELECTOR', 3);
define('CUSTOMFIELD_TYPE_HIDDEN', 4);
define('CUSTOMFIELD_TYPE_DATESELECTOR', 5);
// --- #
...
/*
 * AK-LS: 
 * Crea un nuovo record nella tabella facetoface_sessions_docenti
 * @param: $session ID dell'edizione, teachers array contenente gli ID dei docenti (user) da associare all'edizione
 */
function facetoface_add_session_docenti($session, $teacher) {
    global $DB;	
    $status = true;
/*
	...
	$data->userid = $t;	
	$status = $DB->insert_record('facetoface_sessions_docenti', $data);
	if (!$status) break;
    }	
*/
    $data = new stdClass();
...
/*
 * AK-LS:
* Aggiorna i record nella tabella facetoface_sessions_docenti
* @param: $session è l'ID dell'edizione, teachers è un array di array contenente gli ID dei docenti (user) da associare all'edizione
* @return: void
*/
function facetoface_update_session_docenti($session, $teachers) {
	global $DB;

	$status = true;
    $transaction = $DB->start_delegated_transaction();
    try {
        $DB->delete_records('facetoface_sessions_docenti', array('sessionid' => $session));
        ...
        foreach ($teachers as $t) {
            ...
            $DB->insert_record('facetoface_sessions_docenti', $data);
        }
        $transaction->allow_commit();
...
function facetoface_delete_session($session) {
    ...
    // 2017 11 28
    $DB->delete_records('feedback_item', array('teacher_item' => $session->id));
    $DB->delete_records('facetoface_sessions_docenti', array('sessionid' => $session->id));
    // 2017 11 28
...
function facetoface_email_substitutions($msg, $facetofacename, $reminderperiod, $user, $data, $sessionid) {
    ...
    if ($data->datetimeknown) {
        ...
        foreach ($data->sessiondates as $date) {
            ...
            // 19 03 2019 $alldates .= userdate($date->timestart, get_string('strftimedate')).', ';
            $alldates .= ' dal '.userdate($date->timestart, get_string('strftimedate')).', ';
            $alldates .= ' alle '.userdate($date->timestart, get_string('strftimetime')).
                         ' al '.userdate($date->timefinish, get_string('strftimedate')).
                         ' alle '.userdate($date->timefinish, get_string('strftimetime'));
                // 19 03 2019 ' to '.userdate($date->timefinish, get_string('strftimetime'));
        }
    }
    else {
        // Wait-listed session
    ...
    // Aggiunta del 19 03 2019.
    $nomecorso = estrai_nome_corso_da_id_sessione($sessionid);

    // 19 03 2019 $msg = str_replace(get_string('placeholder:facetofacename', 'facetoface'), $facetofacename, $msg);
    $msg = str_replace(get_string('placeholder:facetofacename', 'facetoface'), $nomecorso, $msg); // 19 03 2019
...
/*
 * AK-LS: questa funzione serve per recuperare tutti gli utenti in un
 * particolare status specificato (o un insieme di status eterogeneo)
 * @param 	$sessionid è l'id della sessione di riferimento
 * 			$status è un array di stati per cui si vuole ricercare gli utenti
 * @return 	un elenco di utenti in quella sessione e con lo stato ricercato. L'oggetto
 * 			restituito contiene sia il recordset di utenti che il numero di
 * 			item presenti
 */
function facetoface_get_users_instatus($sessionid, $status, $data=null, $substitute=false) {
	global $CFG, $DB;
	...
	if ($data->column == 'nome') $data->column = 'lastname';
	
	$statuscode = implode(",",$status);
		
	if (!$substitute) { // Consuntivo
		$selectClause = ", IF(ss.presenza IS NULL, 0, ss.presenza) as presenza, IF(ss.va IS NULL, 0, ss.va) as va, ss.stores";
		$whereClause = "AND ss.statuscode IN (".$statuscode.")";
	} else { // Iscrizione/Sostituzione
		//$selectClause = ",ss.f2_user_changes, su.f2_note"; // 2019 05 02 spostato nel corpo della query
		$selectClause = ", su.f2_note";
		$whereClause = "
        		AND (
					ss.statuscode IN (".$statuscode.")
					OR (
						ss.statuscode = ".MDL_F2F_STATUS_USER_CANCELLED." 
						AND ss.f2_substituted = 1
					)
				)";
		if ($data->column == 'stato_iscrizione') $data->column = 'statuscode';		
	}

        // 2018 03 30
        $cercasunome = '';
        if (($data->nome != '') || (!empty($data->nome))) {
            //$nome_esc = mysqli_real_escape_string($data->nome);
            $nome_esc = $data->nome;
            $catena1 = ' (CONCAT(u.lastname,\' \',u.firstname) LIKE \'%'.$nome_esc.'%\')';
            $catena2 = ' (CONCAT(u.firstname,\' \',u.lastname) LIKE \'%'.$nome_esc.'%\')';
            $cercasunome = ' AND ('.$catena1.' OR '.$catena2.') ';
        }
        // 2018 03 30
	...
	$sql = "
			...
			WHERE
				...
				/*AND 
				(
				CONCAT(u.lastname,' ',u.firstname) LIKE '%".mysqli_real_escape_string($data->nome)."%'
				OR CONCAT(u.firstname,' ',u.lastname) LIKE '%".mysqli_real_escape_string($data->nome)."%'
				)*/
			ORDER BY
				".$data->column." ".$data->sort;	
				
	$objResult = new stdClass();
	$objResult->dati = $DB->get_records_sql($sql, array($sessionid), $data->page*$data->perpage,$data->perpage);
	$objResult->count = $DB->count_records_sql("SELECT COUNT(*) FROM ($sql) as TMP", array ($sessionid, $statuscode));
...
function excel_sessions_seats($template, $courses) {
	ob_start();
	global $CFG;

$type_course = $_GET['coursetype'];
	...
	// Proprietà del file
	$objPHPExcel->getProperties()->setCreator("CSI Piemonte")
								->setLastModifiedBy(time())
								->setTitle("Posti riservati per edizione")
								->setSubject("CSI Piemonte")
								->setDescription("Elenco delle edizioni con il numero di posti riservati e consumati, per la direzione")
								->setKeywords("posti riservati posti consumati edizione direzione")
								->setCategory("CSI Piemonte");
	...
	for ($i = 0; $i <= count($template); $i++) {
		if (isset($keys[$i]))
			...
			->setCellValueByColumnAndRow($i,$row,$template[$keys[$i]]);
	}
	...
	foreach ($courses as $course) {
		for ($i = 0; $i <= count($keys); $i++) {
			if (isset($keys[$i]) && isset($course->$keys[$i])) {
				if ($keys[$i] === 'edition_timestart' || $keys[$i] === 'edition_timefinish'){
					if($course->$keys[$i])
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i,$row,date('d/m/Y',(string)$course->$keys[$i]));
					else
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i,$row,'');
				}
				else
					$objPHPExcel->getActiveSheet()
					->setCellValueByColumnAndRow($i,$row,(string)$course->$keys[$i]);
			}
		}
		$row++;
	}
	...
	// Titolo del file
	if($type_course == CONST_CORSO_PROGRAMMATO)
		$title = 'corsi_programmati';
	else if($type_course == CONST_CORSO_OBBIETTIVO)
		$title = 'corsi_obiettivo';
	else
		$title = 'Seats';
...
function facetoface_send_request_notice($facetoface, $session, $userid) {
    ...
    // Aggiunta del 19 03 2019.
    //$nomecorso = estrai_nome_corso_da_id_sessione($session->id);
 ...
 *
 */
function facetoface_update_signup_status($signupid, $statuscode, $createdby, $note='', $grade=NULL, $presenza=null, $va=null, $stores=null) {
    global $DB, $USER;
	//AK-DL: upgrade $USER
    $timenow = time();
	...
	$signupstatus->f2_user_changes = $USER->id;// AK-DL: upgrade  $signupstatus->f2_user_changes = $USER->id;
	$signupstatus->presenza = $presenza; // AK-LS: giorni per cui viene consuntivata la presenza dello studente al corso
	$signupstatus->va = $va; // AK-LS: valutazione conseguita per il corso
	$signupstatus->stores = $stores; // AK-LS: congelamento del consuntivo per l'utente
...
function facetoface_send_notice($postsubject, $posttext, $posttextmgrheading,
    ...
    if ($notificationtype & MDL_F2F_ICAL) {
        if (get_config(NULL, 'facetoface_oneemailperday')) {
            ...
            foreach ($sessiondates as $sessiondate) {
                ...
                //$subject = facetoface_email_substitutions($postsubject, $nomecorso, $facetoface->reminderperiod, $user, $session, $session->id);
    ...
    //$postsubject = facetoface_email_substitutions($postsubject, $nomecorso, $facetoface->reminderperiod, $user, $session, $session->id);
...
/*
 * AK-LS: 
 * Riscrittura della funzione di consuntivazione delle presenze ::facetoface_take_attendance::
 * Oltre ad essere valorizzato il grade dell'utente in base allo statuscode 
 * sono popolate i campi personalizzati per CSI, presenza e verifica dell'apprendimento
 * @param $data informazioni passate in POST alla pagina che chiama questa funzione
 * @return true o false
 */
function facetoface_take_attendance_csi($data) {
	global $USER;
	...
	$users = (isset($data->id_attendees)) ? $data->id_attendees : null;
	if (!is_null($users)) {
		$idx_presenza = "presenza_".$data->formname;
		$idx_va = "va_".$data->formname;
		$presenza = $data->$idx_presenza;
		$va = $data->$idx_va;
		
		$status = ($va == "_" && $presenza == 0) ? MDL_F2F_STATUS_NO_SHOW : MDL_F2F_STATUS_FULLY_ATTENDED;
		...
		foreach ($users as $u) {
			facetoface_update_signup_status($u, $status, $USER->id, '', $grade, $presenza, $va);
		
			if (!facetoface_take_individual_attendance($u, $grade)) {
				error_log("F2F: could not mark '$u' as ".$value);
				return false;
			}
		}
...
/*
 * AK-LS:
 * Questa funzione serve per archiviare le informazioni corso/sessione/edizione/utente nello storico 
 * corsi. Inoltre deve congelare lo status dell'utente all'interno dell'edizione facetoface, in modo
 * che non sia più modificabile in fase di consuntivazione.
 * In caso di fallimento in fase di INSERT/UPDATE viene fatto un rollback del DB allo stato precedente
 * @param informazioni passate in POST alla pagina che chiama questa funzione
 * @return true o false
 */
function facetoface_stores_csi($data) {
	global $USER, $CFG, $DB;

	require_once($CFG->dirroot.'/f2_lib/management.php');
	define('CONST_CORSO_OBBIETTIVO', 1);
	...
	if (!is_null($users)) {
		...
		$utenti_non_archiviati = "";
		// Storing-phase
		$storing = array();
		foreach ($users as $u) {
			$objStore = new stdClass();
			$objStore = $DB->get_record_sql("
					SELECT 
						u.id AS userid,
						fsis.id AS signupstatus,
						u.idnumber AS matricola,
						u.lastname AS cognome,
						u.firstname AS nome,
						(SELECT data FROM {user_info_data} uid, {user_info_field} uif WHERE uid.fieldid = uif.id AND uif.shortname LIKE 'sex' AND uid.userid = u.id) AS sesso,
						(SELECT data FROM {user_info_data} uid, {user_info_field} uif WHERE uid.fieldid = uif.id AND uif.shortname LIKE 'category' AND uid.userid = u.id) AS categoria,
						(SELECT data FROM {user_info_data} uid, {user_info_field} uif WHERE uid.fieldid = uif.id AND uif.shortname LIKE 'ap' AND uid.userid = u.id) AS ap,
						u.email AS e_mail,
						(SELECT data FROM {facetoface_session_data} fsd, {facetoface_session_field} fsf WHERE fsf.id = fsd.fieldid AND fsf.shortname LIKE 'editionum' AND fsd.sessionid = $sessionid) AS edizione,
						c.idnumber AS codcorso,
						IF(f2_ac.course_type = ".CONST_CORSO_OBBIETTIVO.", 'O', 'P') AS tipo_corso,
						IF(f.f2session = 0, 0, (SELECT f2_s.numero FROM {f2_sessioni} f2_s WHERE f2_s.id = f.f2session)) AS sessione,
						(SELECT data FROM {facetoface_session_data} fsd, {facetoface_session_field} fsf WHERE fsf.id = fsd.fieldid AND fsf.shortname LIKE 'indirizzo' AND fsd.sessionid = $sessionid) AS sede_corso,
						(SELECT f2_se.descrizione FROM {facetoface_session_data} fsd, {facetoface_session_field} fsf, {f2_sedi} f2_se WHERE fsf.id = fsd.fieldid AND fsf.shortname LIKE 'sede' AND fsd.sessionid = $sessionid AND fsd.data = f2_se.id) AS localita,
						f2_ac.costo,
                                                f2_ac.af,
						f2_ac.to_x,
						(SELECT data FROM {facetoface_session_data} fsd, {facetoface_session_field} fsf WHERE fsf.id = fsd.fieldid AND fsf.shortname LIKE 'sirp' AND fsd.sessionid = $sessionid) AS sirp,
						(SELECT data FROM {facetoface_session_data} fsd, {facetoface_session_field} fsf WHERE fsf.id = fsd.fieldid AND fsf.shortname LIKE 'sirpdata' AND fsd.sessionid = $sessionid) AS sirpdata,
						(SELECT data FROM {facetoface_session_data} fsd, {facetoface_session_field} fsf WHERE fsf.id = fsd.fieldid AND fsf.shortname LIKE 'periodo' AND fsd.sessionid = $sessionid) AS periodo,
						f2_ac.orario,
						c.fullname AS titolo,
						f2_ac.durata,
						IF(f2_ac.flag_dir_scuola = 'D', (SELECT o.fullname FROM {org} o WHERE o.id = f2_ac.id_dir_scuola), (SELECT f2_f.denominazione FROM {f2_fornitori} f2_f WHERE f2_f.id = f2_ac.id_dir_scuola)) AS scuola_ente,
						fsis.presenza,
						f2_ac.determina,
						f2_ac.subaf AS sub_af,
						fsis.va,
						f2_ac.te,
						f2_ac.sf,
						f2_ac.cf,
					...
					JOIN
						{facetoface_signups_status} fsis
					 ON fsi.id = fsis.signupid
					JOIN 
						{user} u
					 ON fsi.userid = u.id
					JOIN 
					...
					WHERE 
						fsi.id = $u
						AND fsi.sessionid = $sessionid
						AND fsis.superceded = 0");
			
			// Direzione dell'utente
			$direzione = get_direzione_utente($objStore->userid);
			$objStore->cod_direzione = $direzione['shortname'];
			$objStore->direzione = $direzione['name'];
			
			// Settore dell'utente
			$settore = get_settore_utente($objStore->userid);
			$objStore->cod_settore = $settore['shortname'];
			$objStore->settore = $settore['name'];

			//Se un utente non ha nessuna direzione e nessun settore non deve essere archiviato
			if(!($direzione || $settore)){
				$utenti_non_archiviati .= $objStore->userid.",";
			}else{		
                        // Padding sull'Edizione
                        $objStore->edizione = str_pad($objStore->edizione, 2, "0", STR_PAD_LEFT);
			
			// Data inizio dell'edizione
			$edzPeriodo = periodo_sessions($sessionid);
			$objStore->data_inizio = $edzPeriodo->timestart;
			
                        // Se nei custom field dell'edizione non era presente l'indirizzo, allora prendo quello del corso
                        if (is_null($objStore->sede_corso) || $objStore->sede_corso == '')
                            $objStore->sede_corso = $objStore->viaente;
                        
			$objStore->codcitta = NULL;
			$objStore->prot = NULL;
			$objStore->note = NULL;
			$objStore->delibera = NULL;
			
			// Calcolo del CODPART		
			$fSoglia = round((2*$objStore->durata/3)*100)/100;
			if ($objStore->presenza >= $fSoglia) {
				switch ($objStore->va) {
					case "3":
					case "4":
					case "5":
						$objStore->codpart = 1;
						$objStore->descrpart = "Esecutivo con verifica";
						$objStore->cfv = $objStore->cf;
						break;
					default:
						$objStore->codpart = 2;
						$objStore->descrpart = "Partecipazione senza verifica";
						$objStore->cfv = 0;
				}
			} elseif ($objStore->presenza > 0) {
				$objStore->codpart = 3;
				$objStore->descrpart = "Partecipazione parziale";
				$objStore->cfv = 0;
			} else {
				$objStore->codpart = 7;
				$objStore->descrpart = "Assente";
				$objStore->cfv = 0;
			}
			
			$objStore->servizio = NULL;
			$objStore->cfa = NULL;
			$objStore->ac = NULL;
			$objStore->lstupd = time();
			
			$storing[] = $objStore;
		}
	}
		$transaction = $DB->start_delegated_transaction();
		foreach ($storing as $objRecord) {// Insert-phase
			if (!is_null($objRecord->presenza) && !is_null($objRecord->va)) { // L'archiviazione ha effetto solo quando presenza e va sono valorizzati
				// Freeze facetoface signups status
				$where = "id = ? AND superceded = 0";
				$whereparams = array($objRecord->signupstatus);
				$DB->set_field_select('facetoface_signups_status', 'stores', 1, $where, $whereparams);
				
				// Dall'oggetto $objRecord vengo eliminati due campi che non devono essere inseriti nello storico
				unset($objRecord->userid);
				unset($objRecord->signupstatus);
				
				
				if (!$DB->insert_record("f2_storico_corsi", $objRecord)) {
					print_error('error:couldnotinsertstorico', 'facetoface');
	...
	$result = new stdClass();
	$result->return = true;
	$result->utenti_non_archiviati = $utenti_non_archiviati;
	return $result;
}
...
function facetoface_get_user_submissions($facetofaceid, $userid, $includecancellations=false) {
    ...
    if (!$includecancellations) {
        // AKTIVE: vengono considerati anche gli utenti che hanno partecipato parzialmente o completamente ad una edizione
        $whereclause .= ' AND ss.statuscode >= ? AND ss.statuscode <= ?';
        $whereparams = array_merge($whereparams, array(MDL_F2F_STATUS_REQUESTED, MDL_F2F_STATUS_FULLY_ATTENDED));
//        $whereclause .= ' AND ss.statuscode >= ? AND ss.statuscode < ?';
...
function facetoface_remove_bookings_from_user_calendar($session, $userid) {
    ...
    //AK-LM: cancellava tutti gli eventi delle sessioni dell'istanza f2f; aggiunta condizione su timestart
    foreach ($session->sessiondates as $date) {
        $params = array($session->facetoface, $userid, $date->timestart);
        $DB->delete_records_select('event', "modulename = 'facetoface' AND
 ...
 */
function facetoface_print_session($session, $showcapacity, $calendaroutput=false, $return=false, $hidesignup=false, $customToExclude=array()) {
    global $CFG, $DB;
    ...
    $customdata = $DB->get_records('facetoface_session_data', array('sessionid' => $session->id), '', 'fieldid, data');
    foreach ($customfields as $field) {
    	if (!empty($customToExclude) && !in_array($field->shortname, $customToExclude)) {
        	$data = '';        
	        if (!empty($customdata[$field->id])) {
	            if (CUSTOMFIELD_TYPE_MULTISELECT == $field->type) {
                ...
                if ($field->shortname == 'sirpdata') {
                    $date = $data == 0 ? 'Data da definire' : userdate($data, get_string('strftimedaydate'));
                    $table->data[] = array(str_replace(' ', '&nbsp;', format_string($field->name)), $date);
                } else {
 ...
 * 
 * AK-LS: aggiunto nuovo parametro $objOptions per la gestione dei valori
 * da passare ad oggetti SELECT e MULTISELECT
 */
function facetoface_add_customfields_to_form(&$mform, $customfields, $alloptional=false, $objOptions=array())
{
    foreach ($customfields as $field) {
        $fieldname = "custom_$field->shortname";

        $options = array();
        if (array_key_exists($fieldname, $objOptions)) {
        	$options = $objOptions[$fieldname];
        } else {        
	        if (!$field->required) {
         ...
         * AK-LS: upgrade face-to-face con nuovi customfields
         */
        case CUSTOMFIELD_TYPE_DATESELECTOR:
        	$mform->addElement('date_selector', $fieldname, $field->name);
        	break;
        case CUSTOMFIELD_TYPE_DATETIMESELECTOR:
        	$mform->addElement('date_time_selector', $fieldname, $field->name);
        	break;
        case CUSTOMFIELD_TYPE_HIDDEN:
        	$mform->addElement('hidden', $fieldname, $field->name);
        	break;
       	// --- #
        default:
        ...
        // AKTIVE: è possibile disabilitare la data di protocollazione (sirpdata) per lasciare il campo vuoto
        if ($fieldname == 'custom_sirpdata') {
            $mform->addElement('checkbox', 'disable_sirpdata', '',
                        " " . get_string('disable_sirpdata', 'facetoface'));
            $mform->setDefault('disable_sirpdata', false);

            $mform->disabledIf('custom_sirpdata', 'disable_sirpdata', 'checked');
        }
...
class facetoface_candidate_selector_csi extends user_selector_base {
	protected $sessionid;
	
	public function __construct($name, $options) {
		$this->sessionid = $options['sessionid'];
		$this->ref = $options['ref'];
		$this->visible = isset($options['visible']) ? $options['visible'] : 0;
		parent::__construct($name, $options);
	}
	
	public function find_users($search) {
		global $CFG, $DB;
		include_once($CFG->dirroot.'/f2_lib/management.php');
		...
		$whereUsersRef = "";		
		if (!is_null($this->ref)) {
			$userslist = get_visible_users_by_userid($this->ref);
			$str_users1 = "";
			foreach ($userslist->dati as $user)
				if (!empty($user->id))
					$str_users1 .= $user->id.', ';
			$str_users1 = substr($str_users1, 0, -2);
			$whereUsersRef = (count($userslist->dati) >0 ) ? "AND mdl_user.id IN ($str_users1)" : "AND mdl_user.id IN (0)";
		}
		
		$whereUsersVisible = "";
		if ($this->visible == USERS_PRENOTATI_VALIDATI) {
			$courseID = $DB->get_field_sql("SELECT f.course FROM {facetoface} f, {facetoface_sessions} fs WHERE f.id = fs.facetoface AND fs.id = $this->sessionid");
			$userspre = $DB->get_records_sql("SELECT userid FROM {f2_prenotati} WHERE courseid = $courseID AND validato_dir = validato_sett AND validato_dir = 1");
			$str_users2 = "";
			foreach ($userspre as $user)
				if (!empty($user->userid))
					$str_users2 .= $user->userid.', ';
			$str_users2 = substr($str_users2, 0 , -2);
			$whereUsersVisible = (count($userspre) > 0) ? "AND mdl_user.id IN ($str_users2)" : "AND mdl_user.id IN (0)";
		}
		
		//$fields      = 'SELECT id, firstname, lastname, idnumber, email';
		$fields      = 'SELECT id, firstname, lastname, idnumber, email, firstnamephonetic, lastnamephonetic, middlename, alternatename ';
...
class facetoface_existing_selector_csi extends user_selector_base {
	protected $sessionid;
	...
	public function find_users($search) {
		global $CFG, $DB;
		include_once($CFG->dirroot.'/f2_lib/management.php');
		include_once($CFG->dirroot.'/f2_lib/course.php');
		...
		if (!is_null($this->ref)) {
			...
			foreach ($userslist->dati as $user)
				if (!empty($user->id))
					$str_users1 .= $user->id.', ';
			$str_users1 = substr($str_users1, 0, -2);
			$whereUsersRef = (count($userslist->dati) > 0) ? "AND u.id IN ($str_users1)" : "AND u.id IN (0)";
		}
		
		$courseID = $DB->get_field_sql("SELECT f.course FROM {facetoface} f, {facetoface_sessions} fs WHERE f.id = fs.facetoface AND fs.id = $this->sessionid");
		$sessiondates = periodo_sessions($this->sessionid);
		
		$sql2 = "
			(SELECT
				DISTINCT fsi.userid as id
			FROM
				{facetoface} f
			JOIN
				{facetoface_sessions} fs
			...
			WHERE
				f.course <> $courseID AND
				fss.superceded = 0 AND
				fss.statuscode = ".MDL_F2F_STATUS_BOOKED." AND 
				(((SELECT FROM_UNIXTIME($sessiondates->timestart,'%Y%m%d')) <= (SELECT FROM_UNIXTIME(fsd.timestart,'%Y%m%d')) AND (SELECT FROM_UNIXTIME(fsd.timestart,'%Y%m%d')) <= (SELECT FROM_UNIXTIME($sessiondates->timefinish,'%Y%m%d'))) OR 
				((SELECT FROM_UNIXTIME($sessiondates->timestart,'%Y%m%d')) <= (SELECT FROM_UNIXTIME(fsd.timefinish,'%Y%m%d')) AND (SELECT FROM_UNIXTIME(fsd.timefinish,'%Y%m%d')) <= (SELECT FROM_UNIXTIME($sessiondates->timefinish,'%Y%m%d'))) OR 
				((SELECT FROM_UNIXTIME(fsd.timestart,'%Y%m%d')) <= (SELECT FROM_UNIXTIME($sessiondates->timestart,'%Y%m%d')) AND (SELECT FROM_UNIXTIME(fsd.timefinish,'%Y%m%d')) >= (SELECT FROM_UNIXTIME($sessiondates->timefinish,'%Y%m%d'))))
			) as T2";
		...
		$fields = "
			SELECT
                ...
                T1.idnumber,
				T1.email,
                T1.firstnamephonetic, T1.lastnamephonetic, T1.middlename, T1.alternatename, 
				T1.discountcost,
				T1.discountcode,
				T1.notificationtype,
				T1.facetofaceid,
				...
				T1.statuscode,
				T1.timecreated,
				IF(T2.id IS NULL, 0, 1) as otherbooking FROM";

		$countfields = 'SELECT COUNT(1) FROM';
		$sql = "(
			SELECT
				u.id,
				su.id AS submissionid,
		...
		$order = " ORDER BY T1.timecreated ASC";
		...
		if (!$this->is_validating()) {
			$potentialmemberscount = $DB->count_records_sql($countfields . $sql . " LEFT JOIN " . $sql2 . " ON (T1.id = T2.id) ", $params);
			if ($potentialmemberscount > 100) {
				return $this->too_many_results($search, $potentialmemberscount);
			}
		}
	
		$availableusers = $DB->get_records_sql($fields . $sql . " LEFT JOIN " . $sql2 . " ON (T1.id = T2.id) " . $order, $params);
...
class facetoface_potential_teachers_selector_csi extends user_selector_base {
  protected $sessionid;
  ...
  public function find_users($search) {
    ...
    //list($wherecondition, $params) = $this->search_sql($search, '{f2_formatore}');
    //list($wherecondition, $params) = $this->search_sql($search, '{user}');
...
/*    $whereUsersRef = "";
		...
		$whereUsersVisible = "";
		if ($this->visible == TEACHERS_PRENOTATI_VALIDATI) {
			$courseID = $DB->get_field_sql("SELECT f.course FROM {facetoface} f, {facetoface_sessions} fs WHERE f.id = fs.facetoface AND fs.id = $this->sessionid");
			$userspre = $DB->get_records_sql("SELECT userid FROM {f2_prenotati} WHERE courseid = $courseID AND validato_dir = validato_sett AND validato_dir = 1");
			$str_users2 = "";
			foreach ($userspre as $user)
    ...
    //$fields = 'SELECT u.id, u.firstname, u.lastname, u.idnumber, u.email';
    $fields = 'SELECT u.id, u.firstname, u.lastname, u.idnumber, u.email, u.firstnamephonetic, u.lastnamephonetic, u.middlename, u.alternatename';
    $countfields = 'SELECT COUNT(1)';
    $notIn = " AND u.id NOT IN (SELECT userid FROM {facetoface_sessions_docenti} WHERE sessionid = $this->sessionid) ";
    //$sql = " FROM {f2_formatore} f2_f, {user} u WHERE f2_f.usrid = u.id $notIn ORDER BY u.lastname, u.firstname";
    $sql = " FROM {f2_formatore} f2_f, {user} u WHERE $wherecondition AND f2_f.usrid = u.id $notIn ORDER BY u.lastname, u.firstname";
    //$sql = " FROM {f2_formatore} f2_f, {user} u WHERE $wherecondition $whereUsersVisible $whereUsersRef AND f2_f.usrid = u.id $notIn ORDER BY u.lastname, u.firstname";
    //$params = array();
    if (!$this->is_validating()) {
      $potentialmemberscount = $DB->count_records_sql($countfields . $sql, $params);
      if ($potentialmemberscount > 1001) {
        return $this->too_many_results($search, $potentialmemberscount);
      }
    }
    $availableteachers = $DB->get_records_sql($fields . $sql, $params);
    if (empty($availableteachers)) {
      return array();
    }
    $groupname = get_string('potentialusers', 'role', count($availableteachers));

    return array($groupname => $availableteachers);
  }
...
class facetoface_existing_teachers_selector_csi extends user_selector_base {
  protected $sessionid;
...
//    include_once($CFG->dirroot.'/f2_lib/course.php');
    ...
    $fields = 'SELECT u.id, u.firstname, u.lastname, u.idnumber, u.email, u.firstnamephonetic, u.lastnamephonetic, u.middlename, u.alternatename';
    $countfields = 'SELECT COUNT(1)';
    $sql = " FROM {facetoface_sessions_docenti} fsd, {user} u WHERE $wherecondition AND fsd.userid = u.id AND fsd.sessionid = $this->sessionid ORDER BY u.lastname, u.firstname";
/*
        $sessiondates = periodo_sessions($this->sessionid);

        $q = "SELECT  u.id, u.firstname, u.lastname
              FROM {f2_formatore} f2_f, {user} u
              WHERE f2_f.usrid = u.id
              ORDER BY u.lastname, u.firstname";

        $qc = "SELECT COUNT(u.id) 
               FROM {f2_formatore} f2_f, {user} u 
               WHERE f2_f.usrid = u.id 
               ORDER BY u.lastname, u.firstname";
*/
    if (!$this->is_validating()) {
      //$potentialteacherscount = $DB->count_records_sql($qc);
      $existingteacherscount = $DB->count_records_sql($countfields . $sql, $params);
      if ($existingteacherscount > 100) {
        return $this->too_many_results($search, $existingteacherscount);
      }
    }

    //$availableteachers = $DB->get_records_sql($q);
    $existigteachers = $DB->get_records_sql($fields . $sql, $params);

    if (empty($existigteachers)) {
      return array();
    }

    $groupname = get_string('existingusers', 'role', count($existigteachers));
    return array($groupname => $existigteachers);
  }
    ...
    public function find_users($search) {
        ...
        $sql2 = "
        	...
        	WHERE
        		f.course <> $courseID AND
       			fss.superceded = 0 AND
        		fss.statuscode = ".MDL_F2F_STATUS_BOOKED.") as T2";
...
/*
       ...
       SELECT 
    T1.id as userid,
    IF(T2.userid IS NULL, 0, 1) as otherbooking,
    T2.submissionid,
    T2.firstname,
    T2.lastname,
    T2.email,
...
FROM
    (SELECT 
        u.id
    FROM
        mdl_facetoface_signups_status fss, mdl_facetoface_signups fs, mdl_user u
    WHERE
        fss.signupid = fs.id
            AND fs.userid = u.id
            AND u.deleted <> 1
            ...
            AND fs.sessionid = ".$this->sessionid.") as T1
        LEFT JOIN
    (SELECT 
        *
    FROM
        ...
        concat(fsig.userid, '_', fsd.id) as num_id,
            ...
            fsd.timefinish
    FROM
        mdl_facetoface_signups fsig, mdl_facetoface_signups_status fssig, mdl_facetoface_sessions_dates fsd, mdl_user u, mdl_facetoface_sessions fs, mdl_facetoface f, mdl_course c
    WHERE
        fs.id = fsig.sessionid
            AND f.id = fs.facetoface
            AND f.course = c.id
            AND u.id = fsig.userid
            AND fssig.signupid = fsig.id
            AND fsd.sessionid = fsig.sessionid
            AND fssig.statuscode = ".MDL_F2F_STATUS_BOOKED."
            AND u.deleted <> 1
            AND u.confirmed = 1
            AND fsig.userid in (SELECT 
                fsig.userid
            FROM
                mdl_facetoface_signups fsig, mdl_facetoface_signups_status fssig
            WHERE
                fsig.sessionid = ".$this->sessionid."
                    AND fssig.signupid = fsig.id)
            AND exists( SELECT 
                *
            FROM
                mdl_facetoface_sessions_dates fsd2
            WHERE
                fsd2.sessionid = ".$this->sessionid."
                    AND ((fsd.timestart >= fsd2.timestart
                    AND fsd.timestart <= fsd2.timefinish)
                    OR (fsd.timefinish >= fsd2.timestart
                    AND fsd.timefinish <= fsd2.timefinish)
                    OR (fsd.timestart <= fsd2.timestart
                    AND fsd.timefinish >= fsd2.timefinish)))) tmp
    WHERE
        tmp.sessionid <> ".$this->sessionid." AND tmp.userid <> 1
    ORDER BY tmp.userid ASC) as T2 ON T1.id = T2.userid";
...
*/
/**
 * Ritorna i valori di un corso a cui è associata l'edizione
 *
 * @param integer $id_session
 * @return stdClass
 */
function get_course_by_session($id_session){
	global $DB;
...
*/
/**
 * Restituisce tutti gli utenti associati alla sessione con stato $statuscode
 *
 * Return all users of session
 * @statuscode name es. MDL_F2F_STATUS_BOOKED
 * @param integer $id_edizione
 * @return array(stdClass Object())
 */
function get_user_session_by_status ($id_edizione,$statuscode){
	global $DB,$CFG;
	
	$whereclause = " fsf.superceded != 1 AND fsf.statuscode = ".$statuscode;
    ...
    $sql = "
			...
			FROM
				...
				{facetoface_signups_status} fsf
			WHERE
				fsf.signupid = fs.id					AND
				fs.sessionid = ".$id_edizione."		AND
				".$whereclause
			;
...
/**
 * Get list of users attending a given session
 * ritorna la lista degli utenti iscritti e sostituiti a una sessione
 *
 * @access public
 * @param integer Session ID
 * @return array
 */
/*
function facetoface_get_attendees_substituted($sessionid) {
    global $CFG,$DB;
    $records = $DB->get_records_sql("
        SELECT
            u.id,
        ...
        ORDER BY
            sign.timecreated ASC,
            ss.timecreated ASC
    ", array ($sessionid, MDL_F2F_STATUS_BOOKED, MDL_F2F_STATUS_WAITLISTED, $sessionid, MDL_F2F_STATUS_BOOKED,MDL_F2F_STATUS_USER_CANCELLED));
...
*/
/**
 * DEPRECATED: use function facetoface_get_users_instatus
 * 
 * Get list of users attending a given session
 * ritorna la lista degli utenti iscritti e sostituiti a una sessione
 * se $data = null ritorna la lista completa senza restrizioni
 *
 * @access public
 * @param integer Session ID
 * @data stdClass 
 * @data $data->page=0; numero di pagina 
 * @data $data->perpage=0; numero elementi per pagina 
 * @data $data->column='nome'; colonna per cui deve essere fatto l'ordinamento 
 * @data $data->sort='ASC'; tipo di ordinamento 
 * @return array
 */
function facetoface_get_attendees_substituted($sessionid,$data=null) {
    global $CFG,$DB;
    ...
    $sql_query = "
		...
		AND 
			(
				CONCAT(u.lastname,' ',u.firstname) LIKE '%".mysql_escape_string($data->nome)."%'
			OR	CONCAT(u.firstname,' ',u.lastname) LIKE '%".mysql_escape_string($data->nome)."%'
			)
        AND (
			ss.statuscode = ?
			OR 
	...
	$results = $DB->get_records_sql($sql_query,array($sessionid, MDL_F2F_STATUS_WAITLISTED, $sessionid, MDL_F2F_STATUS_BOOKED,MDL_F2F_STATUS_USER_CANCELLED),$data->page*$data->perpage,$data->perpage);
	
	$results_count=$DB->count_records_sql("SELECT count(*) from ($sql_query) as tmp",array($sessionid, MDL_F2F_STATUS_WAITLISTED, $sessionid, MDL_F2F_STATUS_BOOKED,MDL_F2F_STATUS_USER_CANCELLED));
	$return				= new stdClass;
...
/**
/**
 * Inserisce il flag 1 a substituted nella tabella facetoface_signups_status
 * in questo modo si capisce che l'utente è stato sostituito
 *
 * @access public
 * @param integer session_id l'id della sessione
 * @param integer $removeuser l'id dell'utente a cui bisogna impostare il flag substituted

 * @return bool true
 */
 function update_substituted($session_id,$removeuser){
  global $DB;
  
		$id_substituted = $DB->get_record_sql("SELECT fss.id FROM {facetoface_signups} fs , {facetoface_signups_status} fss WHERE fss.signupid = fs.id AND fs.sessionid = ".$session_id." AND fs.userid = ".$removeuser." AND fss.superceded = 0 AND fss.statuscode = 10 ");
		
		$parametro= new stdClass();
		$parametro->id = $id_substituted->id;
		$parametro->f2_substituted = 1;

		$result=$DB->update_record('facetoface_signups_status', $parametro);
		return $result;
}
 // --- #
...
/**
/**
 * Inserisce le note nella tabella nella tabella facetoface_signups
 *
 * @param integer session_id l'id della sessione
 * @param array() $parametri la chiave è l'indice dell'array è dell'utente e il valore associato all'indice è la nota
 * es.  Array ( [4019] => nota1 [7069] => nota2 )
 * @return integer 1 se è andato a buon fine altrimenti 0
 */
 function update_note_facetoface_signups ($session_id,$parametri){
  global $DB;

$error=1;
	foreach($parametri as $key => $parametro){

		$id_facetoface_signups = $DB->get_record_sql("SELECT 
													fs.id 
												FROM 
														{facetoface_signups} fs 
												WHERE 
														fs.sessionid = ".$session_id." AND 
														fs.userid = ".$key);
	
		$data = new stdClass();
		$data->id = $id_facetoface_signups->id;
		$data->f2_note = $parametro;

		$result=$DB->update_record('facetoface_signups', $data);
		if(!$result){$error=0;}
	}
	return $error;
}
...
*/
/**
 * Restituisce l'id facetoface_signups
 *
 * Return all users of session
 * @statuscode name es. MDL_F2F_STATUS_BOOKED
 * @param integer $session_id
 * @param integer $userid
 * @return stdClass() $facetoface_signups
 */
function get_facetoface_signups ($session_id,$userid){
	global $DB;
	
	$facetoface_signups = $DB->get_record_sql("SELECT 
														* 
												FROM 
														{facetoface_signups} fs 
												WHERE 
...
/**
/**
 * Inserisce il flag (f2_send_notif)  nella tabella nella tabella facetoface_signups
 *
 * @param integer id_facetoface_signups l'id del facetoface_signups
 * @param int $flag il parametro da inserire 
 * @return integer 1 se è andato a buon fine altrimenti 0
 */
 function update_f2_send_notif_facetoface_signups ($id_facetoface_signups,$flag){
  global $DB;

$error=1;
	
		$data = new stdClass();
		$data->id = $id_facetoface_signups;
		$data->f2_send_notif = $flag;
...
/**
/**
 * Ritorna la lista degli utenti con la mail dummy
 *
 * @param array() $list_user lista degli utenti
 * @return array(stdClass())
 */
function get_dummy_email($list_user){
	global $DB;

	$users = "";
	foreach($list_user as $id_user){
		$users = $id_user->userid.",".$users;	
	}

	$value_mail = get_parametri_by_prefix('p_f2_dummy_email');
	
	$mail_dummy = $value_mail['p_f2_dummy_email']->val_char;
 ...
 * AK-LS
 * 
 * Se la sessione � aperta per l'anno formativo in corso:
 * Supervisore, Supervisiore di Consiglio e Referente di Direzione 
 * possono iscrivere e cancellare utenti, indipendentemente dal tipo di Corso
 * 
 * Se la sessione � chiusa, invece, solo il Supervisore ed il 
 * Supervisore di Consiglio possono procedere al task d'iscrizione
 * 
 * @param id corso, id edizione, tipologia di corso (programmato | obbiettivo)
 * @return TRUE o FALSE se l'utente (il ruolo) pu� iscrivere/cancellare
 */
function can_booking($courseID, $editionID, $coursetype) {
	global $DB, $CFG, $USER;
	...
	// Ruolo di sistema
	//$roles = get_user_roles(get_context_instance(CONTEXT_SYSTEM), $USER->id);
	$roles = get_user_roles(context_system::instance(), $USER->id);
		
	// Ruoli nel corso
	$context = context_course::instance($courseID);
	$courseroles = get_user_role_in_context($USER->id, $context->id);
	
	// Somma dei ruoli globali dell'utente e di quelli assunti nel contesto del corso
	$allRoles = array();
	foreach ($roles as $role)
		$allRoles[$role->roleid] = $role->shortname;
	foreach ($courseroles as $courserole)
		$allRoles[$courserole->roleid] = $courserole->shortname;
	
	if ($coursetype == C_PRO) {
		$sessionOpen = $DB->get_field_sql("
			SELECT 
				IF(f2_s.stato = 'a', 1, 0) as stato
			FROM 
				{facetoface} f
			JOIN 
				{facetoface_sessions} fs 
			...
			JOIN 
				{f2_sessioni} f2_s 
			 ON f.f2session = f2_s.id 
			WHERE 
				fs.id = $editionID");
		if (!$sessionOpen) {			
			if (in_array("supervisore", $allRoles) || in_array("supervisoreconsiglio", $allRoles) || is_siteadmin($USER->id)) return true;
			else return false;
		} else return true;
	} else {
            // i supervisori possono sempre iscrivere
                if (in_array("supervisore", $allRoles) || in_array("supervisoreconsiglio", $allRoles) || is_siteadmin($USER->id)) {
                    return true;
                }
                $today = time();
                $timestart = $DB->get_field('facetoface_sessions_dates', 'timestart', array('sessionid' => $editionID), IGNORE_MULTIPLE);
                $timestart_midnight = make_timestamp(date('Y', $timestart), date('m', $timestart), date('d', $timestart), 0, 0, 0);
                // se la data odierna viene prima delle ore 00:00:00 del giorno relativo alla data di inizio dell'edizione
                // allora le iscrizioni sono aperte
//                print_r('</br>$today: '.$today);
//                print_r('</br>$timestart_midnight: '.$timestart_midnight);
//                print_r('</br>$today: '.userdate($today, get_string('strftimedatetime', 'langconfig')));
//                print_r('</br>$timestart_midnight: '.userdate($timestart_midnight, get_string('strftimedatetime', 'langconfig')));
                if ($today < $timestart_midnight) {
                    if (in_array("referentedirezione", $allRoles)) {
                        return true;
                    } else {
                        return false;
                    }
 ...
 * AK-LS
 * 
 * Se la sessione in esame per l'anno formativo in corso � chiusa 
 * l'operazione di sostituzione in un corso programmato pu� essere 
 * fatta dal Supervisore, il Supervisore di Consiglio ed il Referente 
 * di Direzione.
 * 
 * Nel caso la sessione sia aperta od il corso sia di tipologia
 * obbiettivo anche (nessun controllo sul tipo di ruolo perch�
 * si assume che questa funzione venga chiamata in combinazione
 * con le dovute capability (addattendance, removeattendance)
 * 
 * @param id corso, id edizione, tipologia corso (programmato | obbiettivo)
 * @return TRUE o FALSE
 */
function can_substitute($courseID, $editionID, $coursetype) {
	global $DB, $CFG, $USER;
	...
	if ($coursetype == C_PRO) {
		$sessionClose = $DB->get_field_sql("
				SELECT 
					IF(f2_s.stato = 'c', 1, 0) as stato 
				FROM 
					{facetoface} f
				JOIN 
					{facetoface_sessions} fs 
				...
				WHERE 
					fs.id = $editionID");
		if ($sessionClose) {
			if (in_array("supervisore", $allRoles) || in_array("supervisoreconsiglio", $allRoles) || in_array("referentedirezione", $allRoles) || is_siteadmin($USER->id)) return true;
			else return false;
		} else return true;
	} else return true;	
...
/**
 * Usato per il riordinamento del campo custom "edizione di svolgimento" per le ed. di corso obiettivo.
 * Ordina le edizioni per data d'inizio e riassegna il valore al campo custom 'editionnum'.
 * @global object $DB
 * @param int $facetofaceid Id istanza facetoface
 * @return boolean True se l'aggiornamento ha successo, false altrimenti.
 */
function f2_reorder_sessions_editionnum_by_timestart($facetofaceid) {
    global $DB;
    $success = TRUE;
    $q = "
SELECT 
...
FROM 
(SELECT @rownum:=0) r,
(select s.id, min(sd.timestart) as startdt
  from 
 {facetoface_sessions} s
 ...
 where 
  s.facetoface = ?
 group by s.id 
 order by sd.timestart asc) sub";
    
    if($ordered_sessions = $DB->get_records_sql($q, array($facetofaceid))) {
        $edtionnum_fieldid = $DB->get_field('facetoface_session_field', 'id', array('shortname'=>'editionum'), MUST_EXIST);
        foreach ($ordered_sessions as $session) {
            $success = facetoface_save_customfield_value($edtionnum_fieldid, $session->rownum, $session->id, 'session');
            if(!$success)
                break;
        }
    }
...
/**
 * Estrae i docenti assegnati all'edizione.
 * @global object $DB
 * @param int $sessionid Id sessione facetoface
 * @return boolean True se l'aggiornamento ha successo, false altrimenti.
 */
function get_docenti_assegnati($sessionid) {
    global $DB;
    if ($sessionid > 0) {
        $fields = 'SELECT u.id, u.firstname, u.lastname, u.idnumber, u.email';
        $countfields = 'SELECT COUNT(1)';
        $sql = " FROM {facetoface_sessions_docenti} fsd, {user} u WHERE fsd.userid = u.id AND fsd.sessionid = $sessionid ORDER BY u.lastname, u.firstname"; 
        $params = array();
        $nt = $DB->count_records_sql($countfields . $sql, $params); 
        if ($nt > 0) {
            if ($docenti = $DB->get_records_sql($fields . $sql)) {
                return $docenti;
            } else {
                return 'errore';
            }
 ...
 */
function estrai_nome_corso_da_id_sessione($sid) {
    global $DB;
    if ($sid > 0) {
        $query = 'SELECT c.fullname AS nomecorso '.
                 'FROM mdl_facetoface_sessions AS s, mdl_course AS c, mdl_facetoface AS f '.
                 'WHERE s.id = '.$sid.' AND f.id = s.facetoface AND f.course = c.id';
        if ($nc = $DB->get_record_sql($query)) {
            return $nc->nomecorso;
        } else {
            return '';
        }
