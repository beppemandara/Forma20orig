...
define('F2_FEEDBACK_TEACHERITEM_FLG', 1); // 2017 10 02
...
/**
 * AK-LS: aggiunta parametro opzionale $teacherID
 * 
 ...
 * @return void
 */
function feedback_print_item_complete($item, $value = false, $highlightrequire = false, $teacherID = null) {
    global $CFG;
    if ($item->typ == 'pagebreak') {
        return;
    }
    ...
    //get the instance of the item-class
    $itemobj = feedback_get_item_class($item->typ);
    $itemobj->print_item_complete($item, $value, $highlightrequire, $teacherID);
}
...
function feedback_save_tmp_values($feedbackcompletedtmp, $feedbackcompleted, $userid, $sessionID = null) {
    global $DB;
    ...
    if ($feedbackcompleted) {
        ...
        // AK-LS: aggancio del Feedback completato all'edizione
            $completedSession = new stdClass();
            $completedSession->feedback		= $feedbackcompletedtmp->feedback;
            $completedSession->completed	= $feedbackcompleted->id;
            $completedSession->session		= $sessionID;
            $completedSession->user		= $userid;
// var_dump($completedSession);

            $DB->insert_record('feedback_completed_session', $completedSession);
    }
...
/**
 * AK-LS: aggiunta id dell'edizione per cui dev'essere fatto il salvataggio
 * 
 ...
 */
function feedback_save_values($usrid, $tmp = false, $sessionID = null, $teachers = array(), $docenteID = false) {
    global $DB;
    ...
    if ($usrid == 0) {
        return feedback_create_values($usrid, $timemodified, $tmp, false, $sessionID, $teachers, $docenteID);
    }
 ...
 */
function feedback_create_values($usrid, $timemodified, $tmp = false, $guestid = false, $sessionID = null, $teachers = array(), $docenteID = false) {
    global $DB, $CFG;
    ...
    if ($docenteID) {
        ...
        // AK-LS: utente che risulta aver compilato il Feedback (anche in anonimato)
        if ($feedbackid)
            $usrid = is_user_can_compile_feedback($feedbackid, $sessionID, $usrid);
    }
    ...
    if (!$tmp) {
        $completedSession = new stdClass();
        $completedSession->feedback		= $feedbackid;
        $completedSession->completed	= $completedid;
        $completedSession->session		= $sessionID;
        $completedSession->user			= $usrid;
    
        $completedSessionID = $DB->insert_record('feedback_completed_session', $completedSession);
    }
...
function feedback_delete_completed($completedid) {
    ...
    //AK-LM: delete session completed data
    if($DB->record_exists('feedback_completed_session', $params)) {
        $DB->delete_records('feedback_completed_session', array('completed'=>$completed->id));
    }
...
/**
 * Restituisce l'id dell'istanza del QdG studente per il corso $courseid.
 * NULL altrimenti.
 * @global object $DB
 * @param int $courseid
 * @return mixed Feedback id or NULL
 */
function f2_feedback_get_student_feedback($courseid) {
    global $DB;
    ...
    if($courseid > 0) {
        ...
        list($insql, $inparams) = $DB->get_in_or_equal(array(
            get_string('nome_feedback_studente_aula', 'local_f2_import_course'),
            get_string('nome_feedback_studente_obv', 'local_f2_import_course'),
            get_string('nome_feedback_studente_online', 'local_f2_import_course')
        ));
        $q = "select id from {feedback} where course = ? and name $insql";
        if(!$feedbackid = $DB->get_field_sql($q, array_merge($params, $inparams)))
            $feedbackid = NULL;
    }
...
/**
 * Crea un set di domande inerenti la docenza per il QdG $feedbackid per ciascun docente dell'edizione $sessionid, elencato in $teachers.
 * Applica le modifiche solo se necessarie: elenco dei docenti specificato differente da quello salvato su db.
 * @global object $DB
 * @param int   $sessionid Id dell'edizione
 * @param int   $feedbackid Id del feedback
 * @param array $teachers Id degli utenti docenti
 */
function f2_feedback_set_items_for_teachers($sessionid, $feedbackid, $teachers) {
    global $DB;
    
    if($sessionid > 0 && $feedbackid > 0) {
        $params = array($sessionid, $feedbackid, $sessionid);
        $sql = "
SELECT DISTINCT teacherid 
    FROM {feedback_item} fi
INNER JOIN {facetoface_sessions_docenti} sd ON (sd.userid = fi.teacherid AND sd.sessionid = ?)
WHERE fi.feedback = ? AND fi.teacher_item = ? 
ORDER BY 1";
        $teachers_from_db = $DB->get_fieldset_sql($sql, $params);

        $changes = count(array_diff(
                array_merge($teachers_from_db, $teachers), 
                array_intersect($teachers_from_db, $teachers))
                ) > 0;
        
        if( $changes ) {
            //Recupero l'insieme delle domande inerenti la docenza
            $params = array($feedbackid, F2_FEEDBACK_TEACHERITEM_FLG);
            $sql = "
    SELECT DISTINCT
        label,
        feedback,
    ...
    WHERE feedback = ?
      AND teacher_item >= ?";
            $teacheritems = $DB->get_records_sql($sql, $params);

            if ($teacheritems) {
                $nofitems = count($teacheritems);
            
                $transaction = $DB->start_delegated_transaction();
                try {
                    //Pulizia delle domande inerenti la docenza per edizione
//                    foreach ($teachers_from_db as $teacherid) {
//                        $DB->delete_records('feedback_item', 
//                                array('feedback'=>$feedbackid, 'teacher_item'=>F2_FEEDBACK_TEACHERITEM_FLG, 'teacherid'=>$teacherid));
//                    }
                    $DB->delete_records('feedback_item', 
                                array('feedback'=>$feedbackid, 'teacher_item'=>$sessionid));
                    
                    $sql = "select max(position) from {feedback_item} where feedback = ? and teacher_item >= ?";
                    $maxposition = (int)$DB->get_field_sql($sql, $params, MUST_EXIST);
                    
                    $nofteachers = count($teachers);
                    if($nofteachers > 0) {
                        $nextpos = $maxposition + 1;
                        $sql = "update {feedback_item} set position = position + ? where feedback = ? and position > ?";
                        //Correggo la posizione di ciascuna domanda successiva a maxposition in base a quanto devo aggiungere
                        $DB->execute($sql, array($nofitems*$nofteachers, $feedbackid, $maxposition));
                        foreach ($teachers as $teacherid) {
                            //Aggiungo un set di domande per ogni docente
                            foreach ($teacheritems as $item) {
                                $item->position = $nextpos++;
                                $item->teacherid = $teacherid;
                                $item->teacher_item = $sessionid;
                                $DB->insert_record_raw('feedback_item', $item);
                            }
                        }
                    }
...
/**
 * Crea un set di domande inerenti la docenza per il QdG $feedbackid per ciascun id docente dell'edizione $sessionid.
 * Applica le modifiche solo se necessarie: elenco dei docenti specificato differente da quello salvato su db.
 * @global object $DB
 * @param int   $sessionid Id dell'edizione
 * @param int   $feedbackid Id del feedback
 * @param array $teacherid Id utente docente
 */
function f2_feedback_set_items_for_docente($sessionid, $feedbackid, $teacherid) {
  global $DB;
  if($sessionid > 0 && $feedbackid > 0) {
    $params = array($sessionid, $feedbackid, $sessionid);
    $sql = "SELECT DISTINCT teacherid FROM {feedback_item} fi
            INNER JOIN {facetoface_sessions_docenti} sd ON (sd.userid = fi.teacherid AND sd.sessionid = ?)
            WHERE fi.feedback = ? AND fi.teacher_item = ? ORDER BY 1";
    $teacherid_from_db = $DB->get_fieldset_sql($sql, $params);
    // controllo che l'id docente per questa sessione non sia giàmpostato
    if ($teacherid != $teacherid_from_db->teacherid) {
      // recupero l'insieme delle domande inerenti la docenza
      $params = array($feedbackid, F2_FEEDBACK_TEACHERITEM_FLG);
      $sql = "SELECT DISTINCT label, feedback, template, name, presentation, typ, 
              hasvalue, required, dependitem, dependvalue, options 
              FROM {feedback_item} WHERE feedback = ? AND teacher_item >= ?";
      $teacheritems = $DB->get_records_sql($sql, $params);
      if ($teacheritems) {
        $nofitems = count($teacheritems);
        $transaction = $DB->start_delegated_transaction();
        try {
          ...
          $sql = "UPDATE {feedback_item} SET position = position + ? WHERE feedback = ? AND position > ?";
          // correggo la posizione di ciascuna domanda successiva a maxposition in base a quanto devo aggiungere
          $DB->execute($sql, array($nofitems*1, $feedbackid, $maxposition));
          // aggiungo un set di domande
          foreach ($teacheritems as $item) {
            $item->position = $nextpos++;
            $item->teacherid = $teacherid;
            $item->teacher_item = $sessionid;
...
/**
 * Elimina un set di domande inerenti la docenza per il QdG $feedbackid per ciascun id docente dell'edizione $sessionid.
 * Applica le modifiche solo se necessarie: elenco dei docenti specificato differente da quello salvato su db.
 * @global object $DB
 * @param int   $sessionid Id dell'edizione
 * @param int   $feedbackid Id del feedback
 * @param array $teacherid Id utente docente
 */
function f2_feedback_del_items_for_docente($sessionid, $feedbackid, $teacherid) {
  global $DB;
  if($sessionid > 0 && $feedbackid > 0) {
    /*
    $params = array($sessionid, $feedbackid, $sessionid);
    ...
    $teacherid_from_db = $DB->get_fieldset_sql($sql, $params);
    */
    //$sql = "SELECT COUNT(teacherid) FROM {feedback_item} WHERE feedback = $feedbackid ";
    $conditions = array('feedback' => $feedbackid, 'teacher_item' => $sessionid, 'teacherid' => $teacherid);
    $num = $DB->count_records('feedback_item', $conditions);
    // controllo che l'id docente per questa sessione sia gia' impostato
    //if ($teacherid == $teacherid_from_db->teacherid) {
    if ($num > 0) {
      $transaction = $DB->start_delegated_transaction();
      try {
        $conditions = array("teacher_item" => $sessionid, 'teacherid' => $teacherid, 'feedback' => $feedbackid);
        $DB->delete_records('feedback_item', $conditions);
        $transaction->allow_commit();
        return true;
      } catch(Exception $e) {
        $transaction->rollback($e);
