...
class feedback_item_multichoicerated extends feedback_item_base {
    ...
    public function print_item_complete($item, $value = '', $highlightrequire = false, $teacherID = null) {
        global $OUTPUT;
        $align = right_to_left() ? 'right' : 'left';
        $info = $this->get_info($item);
        $strrequiredmark = '<img class="req" title="'.get_string('requiredelement', 'form').'" alt="'.
        ...
        switch($info->subtype) {
            case 'r':
                ...
                $this->print_item_radio($item, $value, $info, $align, false, $lines, $teacherID);
                break;
            case 'd':
                //$this->print_item_dropdown($item, $value, $info, $align, false, $lines);
                $this->print_item_dropdown($item, $value, $info, $align, false, $lines, $teacherID);
                break;
        }
        echo '</div>';
    }
     ...
     * AK-LS: aggiunta parametro opzionale $teacherID
     */
    private function print_item_radio($item, $value, $info, $align, $showrating, $lines, $teacherID=null) {
        $index = 1;
        $checked = '';
        ...
        echo '<ul>';
        /*
         * AK-LS: se la docenza esiste bisogna creare item
         * appositi per il docente, agganciando nella signature del nome teacher_{id docente}
         */
                ...
                <li class="feedback_item_radio_<?php echo $hv.'_'.$align;?>">
                    <span class="feedback_item_radio_<?php echo $hv.'_'.$align;?>">
                        ...
                        if (is_null($teacherID))
                        	...
                        	echo '<input type="radio" '.
                        			'name="teacher_'.$teacherID.'_'.$item->typ.'_'.$item->id.'" '.
                        			'id="teacher_'.$teacherID.'_'.$item->typ.'_'.$item->id.'_xxx" '.
                        			'value="" checked="checked" />';
                        ?>
                    </span>
                    <span class="feedback_item_radiolabel_<?php echo $hv.'_'.$align;?>">
                    <?php if (is_null($teacherID)) { ?>
                        <label for="<?php echo $item->typ . '_' . $item->id.'_xxx';?>">
                    <?php } else { ?>
                    	<label for="<?php echo 'teacher_'.$teacherID.'_'.$item->typ . '_' . $item->id.'_xxx';?>">
                    <?php } ?>
     ...
     * AK-LS: aggiunta parametro opzionale $teacherID
     */
    private function print_item_dropdown($item, $value, $info, $align, $showrating, $lines, $teacherID=null) {
        if ($info->horizontal) {
            $hv = 'h';
        } else {
            $hv = 'v';
        ...
        <div class="feedback_item_select_<?php echo $hv.'_'.$align;?>">
          ...
          <?php } else { ?>
            <select id="<?php echo 'teacher_'.$teacherID.'_'.$item->typ.'_'.$item->id;?>" name="<?php echo 'teacher_'.$teacherID.'_'.$item->typ.'_'.$item->id;?>">
          <?php } ?>
