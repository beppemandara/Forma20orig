...
$modello_questionario = optional_param('modello_questionario', 0, PARAM_INT);
...
if (!$modello_questionario) {
    ...
    $newformdata = array('id'=>$id,
                    ...
                    'deleteolditems'=>'1',
                    'do_show'=>'edit',
                    'modello_questionario'=>$modello_questionario);
}
...
if ($mform->is_cancelled()) {
    if ($modello_questionario)
        redirect(new moodle_url('/local/f2_course/modello_questionario.php', array('courseid' => $modello_questionario, 'saved' => false)));
    else
...
if (isset($formdata->confirmadd) AND $formdata->confirmadd == 1) {
    ...
    if (isset($formdata->modello_questionario) AND $formdata->modello_questionario != 0)
        redirect(new moodle_url('/local/f2_course/modello_questionario.php', array('courseid' => $modello_questionario, 'saved' => true)));
    else
        redirect('edit.php?id=' . $id);
}
