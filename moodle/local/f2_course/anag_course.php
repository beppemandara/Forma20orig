	...
	public function definition() {
		...
		if(!in_array('anno',$hide_input)){
                        ...
                        if($anag_course->course_type == C_OBB)
                        {
                             ...
                             * quello corrente, più uno nel futuro (solo corsi Obbiettivo)
                             */
		...
		if(!in_array('te',$hide_input)){
			...
			// 2017 11 29 if($anag_course->course_type==C_PRO)
		...
		if(!in_array('id_dir_scuola',$hide_input)){
			...
			$mform->addElement('static', 's_dir_scuola', 
							get_string('dir_scuola', 'local_f2_course').$asterisco, 
...
// 2018 03 06
$mform->addElement('html', '<div id="div_segnaposto_errore" style=\'display\'></div>');
// 2018 03 06
		...
		if(!in_array('viaente',$hide_input)){
			...
			$mform->addHelpButton('viaente', 'viaente', 'local_f2_course');
                        $mform->addRule('viaente', 'inserire un indirizzo', 'required', null, 'client');
		}
		...
		echo '<script type="text/javascript">
				function myValidate(){
                                        if(document.getElementsByName(\'id_dir_scuola\')[0].value == ""){
                                                alert("Inserire un Ente di erogazione");
                                                //return false;
                                                var para = document.createElement("P");
                                                var g = "Ente di erogazione: Campo a compilazione obbligatoria";
                                                //var t = document.createTextNode("Campo a compilazione obbligatoria.");
                                                var t = document.createTextNode(g);
                                                para.appendChild(t);
                                                document.getElementById("div_segnaposto_errore").style.color = "red";
                                                document.getElementById("div_segnaposto_errore").appendChild(para);
                                                return false;
                                        }
