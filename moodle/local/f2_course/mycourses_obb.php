...
function getCourseCompletationFlag($course)
{
    ...
    */
    ...
    else if (!$criteriacomplete && !$ccompletion->timestarted)
    {
        ...
        $flag .= '<i>'.get_string('notyetstartedbis', 'completion').'</i>';
    }
    else
    {
...
//$datisql = get_mycourses_number(NULL, C_OBB, $pagination, $filtercourse_arr, $filteryear_param, $filtersubcategory);
$total_rows = get_mycourses_number(NULL, C_OBB, $pagination, $filtercourse_arr, $filteryear_param, $filtersubcategory);

echo "<br />numero corsi: ".$total_rows."<br />";
//echo "<br />page: ".$pagination['page']."<br />";
//echo "<br />x page: ".$pagination['perpage']."<br />";
    ...
    foreach ($mycourses as $c) {
        ...
        if(!empty($criterias)) //Se sono definiti criteri di completamento
        {
            $val_icon = " <a href=\"{$CFG->wwwroot}/grade/report/user/index.php?id={$c->id}\" title=\"Dettagli valutazione\">".
					"<img src=\"{$CFG->wwwroot}/pix/t/grades.png\" class=\"icon\" alt=\"Dettagli valutazione\" /></a>";
