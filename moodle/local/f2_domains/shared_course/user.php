...
require_once('../assignments/filters/lib.php');
...
$PAGE->set_url(new moodle_url("{$CFG->wwwroot}/local/f2_domains/shared_course/user.php"));
$PAGE->set_title(get_string('gest_assegnazione_ruoli', 'local_f2_domains'));
if (!has_capability('moodle/role:assign', $sitecontext)) {
	print_error('nopermissions', 'error', '', 'assign role to users');
}
...
$returnurl = new moodle_url('/local/f2_domains/shared_course/user.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage, 'page'=>$page));
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('gest_assegnazione_ruoli', 'local_f2_domains'));
...
//$extrasql2 = $extrasql.' AND viewableorganisationid IS NOT NULL AND org2.depthid > 1';
$extrasql2 = $extrasql.' ';
$users = $ufiltering->get_my_users_listing($sort, $dir, $page*$perpage, $perpage, '', '', '',
		$extrasql2, $params, $context);
$usercount = $ufiltering->get_my_users_count(false, true);
$usersearchcount = $ufiltering->get_my_users_count(true, false, '', $extrasql2, $params);
if ($extrasql !== '') {
	echo "<p>$usersearchcount / $usercount ".get_string('users')."</p>";
	$usercount = $usersearchcount;
} else {
	echo "<p>Totale: $usercount ".get_string('users')."</p>";
}
	

$strall = get_string('all');
$baseurl = new moodle_url('/local/f2_domains/shared_course/user.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
...
if (!$users) {
	$match = array();
	echo "<p class=\"msg_feedback_ko\">".get_string('nousersfound')."</p>";
	$table = NULL;
} else {
