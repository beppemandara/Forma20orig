...
require_once($CFG->dirroot.'/mod/facetoface/extends_sessions.php');
...
$sessionid = required_param('sessionid', PARAM_INT);       		  // course id
...
if ($sessionid) { // editing course
    if ($sessionid == SITEID){
        // don't allow editing of  'site course' using this from
        print_error('cannoteditsiteform');
    }
*/
    $session = $DB->get_record('facetoface_sessions', array('id'=>$sessionid), '*', MUST_EXIST);
/*
} else {
    require_login();
...
$course=get_course_by_session($sessionid);
$courseid= $course->id;

  $coursecontext = get_context_instance(CONTEXT_COURSE, $courseid);
    require_capability('moodle/course:update', $coursecontext);
$fullname = $course->fullname;
$baseurl = new moodle_url('/local/f2_notif/template_sessions.php', array('sessionid'=>$session->facetoface));
...
$tab_edit->print_tab_edit_session('notif_sistema');
...
echo '<input type="button" name="Nuovo" value="Nuovo" onClick="show();"/>';
echo '<input type="button" name="Annulla" value="Annulla" onClick="hide();"/>';
...
$mform = new modello_notifica_form('template_sessions.php?courseid='.$courseid.'&sessionid='.$sessionid);
$mform->display();
//FINE FORM
...
//		else{
			...
			echo '<form action="add_template_sessions.php?courseid='.$courseid.'&sessionid='.$sessionid.'" method="post">';
...
echo '<h3>'.get_string('modelli_notifica', 'local_f2_notif').' edizione</h3>';
//TABELLA FONDO PAGINA			
$templates_course = get_template_course($courseid,$sessionid);
...
if($templates_course){
	...
	echo '<form action="delete_template_sessions.php" method="post">';
		echo '<input type="hidden" name="id_corsoid" id="id_corsoid" value='.$courseid.'>';
		echo '<input type="hidden" name="id_sessionid" id="id_sessionid" value='.$sessionid.'>';
		echo '<input type="submit" onClick="return confirmSubmitElimina()"; value="Elimina" />';
			echo html_writer::table($table_notif_course);
		echo '</form>';
}else{
		echo '<b>Non è presente nessun template per questa edizione.</b><br>';
}
echo '</div>';
echo '</div>';	
echo $OUTPUT->box_end();
