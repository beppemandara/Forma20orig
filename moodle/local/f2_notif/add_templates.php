...
$PAGE->navbar->add(get_string('proprieta_notif', 'local_f2_notif'), new moodle_url($url));
$PAGE->set_title(get_string('proprieta_notif', 'local_f2_notif'));
$PAGE->set_heading($SITE->shortname.': '.$blockname);
$PAGE->set_pagelayout('standard');
$PAGE->settingsnav;

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('proprieta_notif', 'local_f2_notif'));
...
echo $str;
		...
		if(!$data->predefinito) 
			...
			//CONTROLLO SE IL TITOLO E' GIA' PRESENTE NELLA TABELLA
			if($DB->record_exists('f2_notif_templates', array('title' => $data->title,'id_tipo_notif' => $data->id_tipo_notif))){
				$exist=1;
			}
			 else{//CONTROLLO SE L'INSERIMENTO E' ANDATO A BUON FINE
				if(insert_template($dati_template))
				$conferma=1;
			 }
}

	if($exist){
		echo '<b>Template non inserito.<br>Il template &egrave; gi&agrave; presente nel database.</b><br>';
	...
	if($conferma){
		echo '<b>Template inserito correttamente.</b><br>';
		echo 'Seleziona il pulsante "Indietro" per tornare alla pagina dei template.<br><br>';
		echo '<a href="templates.php"><button type="button">Indietro</button></a>';
	}
