...
//$Id: managecourse_obb.php 1234 2013-12-11 12:09:34Z l.moretto $
...
$pagename = get_string('managecourse_obb', 'block_f2_apprendimento');
$PAGE->set_context($context);
$PAGE->set_pagelayout('standard');
$PAGE->set_url('/blocks/f2_apprendimento/managecourse_obb.php');
$PAGE->set_title(get_string('managecourse_obb', 'block_f2_apprendimento'));
$PAGE->settingsnav;
$PAGE->navbar->add(get_string('managecourse_obb', 'block_f2_apprendimento'));
$PAGE->set_heading($SITE->shortname.': '.$pagename);
$PAGE->requires->js('/f2_lib/jquery/multiselect/jquery-1.8.2.js');

echo $OUTPUT->header();

$currenttab = 'managecourse_obb';
require('tabs_managecourse.php');

echo $OUTPUT->heading(get_string('managecourse_obb', 'block_f2_apprendimento'));

$isDir = isReferenteDiDirezione($USER->id);
$dirParam = ($isDir) ? 1 : 0;

echo '<p>';
echo html_writer::start_tag('form', array('action' => $CFG->wwwroot.'/blocks/f2_apprendimento/export_managecourse_seats.php?coursetype='.C_OBB.'&dir='.$dirParam, 'class' => 'export_excel', 'method' => 'post'));
...
if ($filtercourse !== '')
{
	...
	$filtercourse_arr['idnumber'] = $filtercourse;
}
if ($filteryear !== '')
{
...
// 2018-01-03 aggiunti altri 2 anni alla select di filtro sull'anno
...
$managecourse = get_managable_course(C_OBB, $viewable_years, $filtercourse_arr, $filteryear_param);
//print_r($managecourse); exit();
$courses = json_course_parsing($managecourse, C_OBB);
...
<div id="coursetype" style="visibility:hidden"><?php echo C_OBB ?></div>
