...
//$Id: aggiungi_anagrafica.php 1163 2013-06-06 11:42:43Z d.lallo $
...
require_login();
//$context = get_context_instance(CONTEXT_SYSTEM); // 21 06 2019
$context = context_system::instance(); // 21 06 2019
...
if ($debug == 1) {
    corsiindlog(array('Id user: '.$userid.' - training: '.$training));
}

$gestionecorsi_url = new moodle_url('/blocks/f2_formazione_individuale/gestione_corsi.php?training='.$training.'');
...
$url = new moodle_url("{$CFG->wwwroot}/blocks/f2_formazione_individuale/aggiungi_anagrafica.php", array('userid' => $userid));
$PAGE->set_url($url);
$PAGE->set_pagelayout('standard');
...
if ($debug == 1) {
    corsiindlog(array('C.F.: '.$codice_fiscale));
}
...
{
		var agree=confirm(\'Tornando indietro non verr&agrave; salvato nessun dato.\nProseguire?\');
			if (agree)
				parent.location=\'user.php?training='.$training.'\';
			else
				return false ;
}
...
$form = new inserisci_daticorso_form(NULL, compact('gestione_corsi', 'training','userid'));
...
if ($form->is_cancelled()) {
    ...
    // 21 06 2019
    if ($debug == 1) {
      corsiindlog(array('offerta_speciale: '.$offerta_speciale));
      corsiindlog(array('note_offerta: '.$data->note_offerta));
    }
    // 21 06 2019
    ...
    // Inserimento record.
    $ret = $DB->insert_record('f2_corsiind', $corso_ind_new); 

if ($debug == 1) {
    corsiindlog(array('Insert f2_corsiind: '.$ret));
}

    // ottengo l'id del nuovo corso
...
if ($debug == 1) {
    corsiindlog(array('START select ID'));
    corsiindlog(array('userid: '.$data->userid));
    corsiindlog(array('costo: '.$data->costo));
    corsiindlog(array('titolo: '.$data->titolo));
    corsiindlog(array('area_formativa: '.$data->af));
    corsiindlog(array('sotto_area_formativa: '.$data->subaf));
    corsiindlog(array('END select ID'));
}
...
if ($debug == 1) {
    corsiindlog(array('id_corsiind: '.$ret));
    corsiindlog(array('data: '.$corso_ind_anfin->data));
    corsiindlog(array('anno: '.$data->finanziario));
    corsiindlog(array('Insert f2_corsiind_anno_finanziario: '.$res_anfin));
}

    if ($ret)
		redirect(new moodle_url("/blocks/f2_formazione_individuale/gestione_corsi.php?training=".$training."&ret=1"));
	else
		redirect(new moodle_url("/blocks/f2_formazione_individuale/gestione_corsi.php?training=".$training."&ret=-1"));
