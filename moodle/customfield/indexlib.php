...
function customfield_list_datatypes() {
    ...
    if ($dirlist = get_directory_list($CFG->dirroot.'/customfield/field', '', false, true, false)) {
        foreach ($dirlist as $type) {
            $datatypes[$type] = get_string('customfieldtype'.$type, 'local_f2_domains');
...
function customfield_edit_category($id, $depthid=0, $redirect, $tableprefix, $type, $subtype, $frameworkid=0, $navlinks=array()) {
    ...
    if ($category = $DB->get_record($tableprefix.'_info_category', array('id' => $id))) {
    ...
    if ($categoryform->is_cancelled()) {
        ...
        if (empty($id)) {
            $strheading = get_string('createnewcategory', 'local_f2_domains');
        } else {
            $strheading = get_string('editcategory', 'local_f2_domains', format_string($category->name));
        }
...
function customfield_edit_field($id, $datatype, $depthid=0, $redirect, $tableprefix, $type, $subtype, $frameworkid=0, $categoryid=0, $navlinks=array()) {
    ...
    if ($fieldform->is_cancelled()) {
        ...
        if (empty($id)) {
            $strheading = get_string('createnewfield', 'local_f2_domains', $datatypes[$datatype]);
        } else {
            $strheading = get_string('editfield', 'local_f2_domains', $field->fullname);
        }
