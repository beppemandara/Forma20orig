<?php //$Id: fieldlib.php 48 2012-08-29 15:10:27Z c.arnolfo $
...
function customfield_load_data(&$item, $type, $tableprefix) {
    ...
    if ($fields = $DB->get_records_select($tableprefix.'_info_field', $depthstr)) {
...
function customfield_definition(&$mform, $itemid, $type, $depthid=0, $tableprefix) {
    ...
    if ($categories = $DB->get_records_select($tableprefix.'_info_category', $depthstr, array(), 'sortorder ASC')) {
        foreach ($categories as $category) {
            if ($fields = $DB->get_records_select($tableprefix.'_info_field', "categoryid=$category->id", array(), 'sortorder ASC')) {
...
function customfield_display_fields($itemid, $tableprefix) {
    ...
    if ($categories = $DB->get_records_select($tableprefix.'_info_category', '', array(), 'sortorder ASC')) {
        foreach ($categories as $category) {
            if ($fields = $DB->get_records_select($tableprefix.'_info_field', "categoryid=$category->id", array(), 'sortorder ASC')) {
                foreach ($fields as $field) {
                    require_once($CFG->dirroot.'/customfield/field/'.$field->datatype.'/field.class.php');
...
function customfield_record($id, $tableprefix) {
    ...
    if ($fields = $DB->get_records_select($tableprefix.'_info_field')) {
        foreach ($fields as $field) {
            require_once($CFG->dirroot.'/customfield/field/'.$field->datatype.'/field.class.php');
            $newfield = 'customfield_'.$field->datatype;
