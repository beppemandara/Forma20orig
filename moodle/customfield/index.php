...
if ($depthid) {
    ...
    $navlinks[] = array('name' => get_string($type.'plural', 'local_f2_domains'), 'link'=> '', 'type'=>'title');
    $navlinks[] = array('name' => get_string($type.'depthcustomfields', 'local_f2_domains'), 'link'=> '', 'type'=>'title');
...
/*        $navlinks[] = array('name'=>get_string('customfieldcategories', 'customfields'), 
    ...
    $pagetitle = format_string(get_string($type.'depthcustomfields', 'local_f2_domains'));
    $navlinks[] = array('name' => get_string('administration'), 'link'=> '', 'type'=>'title');
    $navlinks[] = array('name' => get_string($type.'plural', 'local_f2_domains'), 'link'=> '', 'type'=>'title');
    $navlinks[] = array('name' => get_string($type.'depthcustomfields', 'local_f2_domains'), 'link'=> '', 'type'=>'title');
}
...
require_capability('local/f2_domains:update'.$type.'customfield', $sitecontext);
...
switch ($action) {
   ...
   case 'movefield':
    ...
    case 'deletecategory':
        ...
        print_heading(get_string('deletecategory', 'local_f2_domains'), 'left', '1');
        notice_yesno(get_string('confirmcategorydeletion', 'local_f2_domains', $fieldcount), $redirect, $redirect, $optionsyes, $redirectoptions, 'post', 'get');
        print_footer();
        die;
        break;
    case 'deletefield':
        ...
        print_heading(get_string('deletefield', 'local_f2_domains'), 'left', '1');
        notice_yesno(get_string('confirmfielddeletion', 'local_f2_domains', $datacount), $redirect, $redirect, $optionsyes, $redirectoptions, 'post', 'get');
        print_footer();
        die;
        break;
    case 'editfield':
...
if (!($breadcrumbnavigation)) {
    print_header_simple($pagetitle, '', $navigation, '', null, true);
    print_heading(get_string($type.'depthcustomfields', 'local_f2_domains'));
} else {
    admin_externalpage_setup($type.'frameworkmanage', '', array('type'=>$type));
...
//    print_heading(format_string($depth->fullname)." : ".format_string($category->name), 'left', 1);
    echo $OUTPUT->heading(format_string($depth->fullname)." : ".format_string($category->name));
//    print_heading(get_string('customfields', 'local_f2_domains'));
    echo $OUTPUT->heading(get_string('customfields', 'local_f2_domains'));
}
...
//    echo '<p>' . mitms_captivate_popup('Video help with custom fields and categories', 'setting up custom categories and fields') . '</p>';
    $table = new object();
    $table->head  = array(get_string('customfield', 'local_f2_domains'), get_string('edit'));
    ...
    if ($fields = $DB->get_records_select($tableprefix.'_info_field', "categoryid=$categoryid", array(), 'sortorder ASC')) {
    ...
    if (count($table->data)) {
        ...
        notify(get_string('nocustomfieldsdefined', 'local_f2_domains'));
    }
    echo "<br>";
    // Create a new custom field dropdown menu
    $options = customfield_list_datatypes();
//    echo $OUTPUT->single_select('index.php?type='.$type.'&amp;subtype='.$subtype.'&id=0&amp;action=editfield&amp;frameworkid='.$frameworkid.'&amp;categoryid='.$categoryid.'&amp;depthid='.$depthid.'&amp;datatype=', 'pippo', $options, '', 'choose', 'newfieldform');
    popup_form('index.php?type='.$type.'&amp;subtype='.$subtype.'&id=0&amp;action=editfield&amp;frameworkid='.$frameworkid.'&amp;categoryid='.$categoryid.'&amp;depthid='.$depthid.'&amp;datatype=', $options, 'newfieldform','','choose','','',false,'self',get_string('createnewcustomfield', 'local_f2_domains'));
...
// show custom fields for the given depth
    ...
    echo "<b>".get_string('framework', 'local_f2_domains').":</b> <a href=\"".$CFG->wwwroot."/hierarchy/index.php?type={$type}&frameworkid={$framework->id}\">$framework->fullname</a><br>";
    echo "<b>".get_string('depthlevel', 'local_f2_domains').":</b> $depth->fullname<br>";
